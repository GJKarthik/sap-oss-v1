...
\node[struct,below=of text] (struct) {\textbf{docs/schema/}\\ JSON records + schemas \\ + \tool{validate.py}};
\node[outp,below=of struct] (spec) {\textbf{docs/latex/specs/}\\ this PDF (XeLaTeX)};
...
\midrule
\path{docs/regulations/} (source PDFs)    & Binary source documents. Never edited; content hashes tracked in \path{docs/schema/registry.json}. \\
\path{docs/regulations/machine-readable/} & Per-source extracted text, RAG chunks, and embedding records (re-upsertable into Qdrant via \path{machine-readable/scripts/upsert\_qdrant.py}). \\
\path{docs/regulations/machine-readable/specification-addendum.yaml} & Requirement records (20 requirements) with \texttt{implementation[]} and \texttt{verification[]} pointers. \\
\path{docs/schema/regulations/}           & JSON Schemas (Draft-07): regulation, requirement, conformance-tool, corpus. \\
\path{docs/schema/registry.json}, \path{docs/schema/validate.py} & Master schema index + unified offline validator. \\
\path{docs/latex/specs/regulations/}      & This human-readable PDF (XeLaTeX). \\
\path{docs/regulations/aiverify-main/}    & Vendored AI Verify 2.0 monorepo (Apache-2.0). \\
\path{docs/regulations/moonshot-cicd-main/} & Vendored Moonshot 1.1.0 monorepo (Apache-2.0). \\
\bottomrule
\end{tabularx}
...
\draw[arr] (req) -- (code);
\end{tikzpicture}}
\caption{End-to-end traceability chain verified by \texttt{docs/schema/validate.py}.}
...
\begin{enumerate}
  \item \textbf{Schema conformance.} Every record under
        \path{docs/schema/regulations/}
        (\texttt{regulation.schema.json}, \texttt{requirement.schema.json},
        \texttt{conformance-tool.schema.json},
        \texttt{corpus.schema.json}) validates against its Draft-07 JSON
        Schema using an offline \texttt{referencing} registry.
  \item \textbf{Corpus integrity.} Every entry in
        \path{docs/schema/registry.json} resolves to a schema or record on
        disk, and every record on disk is listed in the registry.
