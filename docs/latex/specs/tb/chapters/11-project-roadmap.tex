...
completes \emph{and} the v2.0 trigger in \cref{sec:v2-trigger} fires).}
...
M06 & M05       & M & TB-PARALLEL-001    & Beta parallel-run gate passes (\cref{sec:tb-parallel-run}). \\
...
C01 & M07 + \cref{sec:v2-trigger} trigger & C & TB-V2-KICKOFF & v2.0 ML anomaly programme kickoff (Chapter~\ref{ch:future-ml}). \\
...
vLLM hosting (\texttt{vllm-only} policy) & Shared platform & M02 & Graceful-degradation ladder (\cref{sec:degradation-ladder}) provides fallback; slip tolerable. \\
...
\begin{itemize}
  \item \textbf{Mandatory refresh}: at the first cycle after GA, and
        ...
        assumptions, unit economics (\cref{sec:tb-unit-economics}),
...
\section{Machine-readable milestone registry}\label{sec:tb-moscow-yaml}
...
\texttt{docs/schema/common/moscow.yaml} for the TB
domain.},label=lst:tb-moscow-yaml]
tb:
  - { id: M01, priority: Must,   title: "Data schema v1.0",        ticket: TB-ENG-001 }
  - { id: M02, priority: Must,   title: "AI pipeline scaffold",    ticket: TB-ENG-002 }
  - { id: M03, priority: Must,   title: "Controls engine",         ticket: TB-ENG-003 }
  - { id: S01, priority: Should, title: "Regulations integration", ticket: REG-INTG-TB-001 }
  - { id: C01, priority: Could,  title: "V2 ML gate",              ticket: TB-V2-GATE }
  # ...
\end{lstlisting}
