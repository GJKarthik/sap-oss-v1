...
\mermaidfig[\textwidth]{diagrams/system-context}%
  {System context. HANA Cloud (green) provides live schema metadata via the MCP
    protocol (blue). The vLLM backend (purple) powers taxonomy expansion and data
    generation. Output is JSONL training data (gold).}%
  {fig:system-context}
