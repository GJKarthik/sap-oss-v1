...
\subsection{Schema Drift Agent and Retraining Trigger}

A dedicated drift agent monitors versioned HANA schema snapshots, dictionary
versions, and training artifact versions. The agent must compare the live schema
registry against the latest approved training baseline on every scheduled check
and on every schema registry commit.

\begin{table}[htbp]
\centering
\caption{Drift Agent Trigger Rules}
\label{tab:drift-agent-triggers}
\begin{tabularx}{\textwidth}{@{}l l X l@{}}
\toprule
\textbf{Trigger} & \textbf{Severity} & \textbf{Required Action} & \textbf{Retraining Scope} \\
\midrule
Column dropped or type changed & CRITICAL & Halt affected query patterns, open schema-drift incident & Full affected domain \\
Column renamed or table added & HIGH & Re-extract schema, regenerate affected taxonomy nodes & Targeted incremental \\
Dictionary value added or changed & HIGH & Regenerate synonym/value mapping tests & Dictionary slice \\
Vocabulary drift over threshold & MEDIUM & Generate new human-facing examples and update aliases & Persona or locale slice \\
Agent prompt drift over threshold & MEDIUM & Refresh agent templates and re-run audience separation checks & Agent slice \\
\bottomrule
\end{tabularx}
\end{table}

\noindent
Every trigger must write an auditable event containing the schema version,
training artifact version, dictionary version, triggering metric, owner, and
retraining decision. Retraining is blocked until the readiness assessment in
Section~\ref{sec:nl-readiness-assessment} returns GREEN for the target audience
or AMBER with an explicit waiver for agent-only publication.

\subsection{Monitoring UI Requirements}

The operations UI must expose schema, specification, and vocabulary drift as
separate surfaces instead of collapsing them into one health indicator. The
minimum dashboard fields are:
\begin{itemize}
  \item current schema version, latest training schema version, and schema delta count;
  \item TTS-EVAL trend, per-metric failures, and confidence-bounded pass/fail state;
  \item unresolved vocabulary terms, dictionary miss rate, and top affected locales;
  \item human vs. agent readiness grade and artifact publication status;
  \item active retraining jobs, blocked deployments, and owner assignment.
\end{itemize}

\section{Drift Metrics and Alert Schema}\label{sec:drift-schema}

Drift detection outputs are serialized using \texttt{tts-drift-metrics.schema.json}
and \texttt{tts-drift-alert.schema.json}. These records enable automated
monitoring and UI visualization of Text-to-SQL health.

\begin{table}[htbp]
\centering
\caption{Fields of \texttt{tts-drift-metrics.schema.json}.}
\label{tab:drift-metrics-fields}
\small
\begin{tabularx}{\linewidth}{@{}l l c X@{}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Notes} \\ \midrule
\texttt{scr}             & number & \checkmark & Schema Coverage Rate (TTS-M01). \\
\texttt{sas}             & number & \checkmark & Semantic Alignment Score (TTS-M05). \\
\texttt{esr}             & number & \checkmark & Execution Success Rate (TTS-M09). \\
\texttt{tts\_eval}       & number & \checkmark & Composite evaluation score (Eq. 18.1). \\
\texttt{drift\_alerts[]} & array  & --         & List of active \texttt{DriftAlert} objects. \\
\texttt{sample\_size}    & integer& \checkmark & Count of production queries evaluated. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Fields of \texttt{tts-drift-alert.schema.json}.}
\label{tab:drift-alert-fields}
\small
\begin{tabularx}{\linewidth}{@{}l l c X@{}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Notes} \\ \midrule
\texttt{alert\_id}       & string & \checkmark & Unique tracking ID. \\
\texttt{severity}       & enum   & \checkmark & \textsc{low}, \textsc{medium}, \textsc{high}, \textsc{critical}. \\
\texttt{drift\_type}     & string & \checkmark & TTS-DRIFT code (e.g., \texttt{TTS-DRIFT-001}). \\
\texttt{metric\_code}    & string & \checkmark & Triggering metric (e.g., \texttt{TTS-M09}). \\
\texttt{delta\_percent}  & number & --         & Percentage deviation from baseline. \\
\texttt{baseline\_id}    & string & \checkmark & ID of the active approved baseline. \\
\bottomrule
\end{tabularx}
\end{table}
