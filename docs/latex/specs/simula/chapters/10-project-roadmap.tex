...
\subsection{Machine-readable milestone registry}\label{sec:sim-moscow-yaml}

Every milestone identifier in this roadmap is also addressable from
...
M06 is especially noteworthy: the reference-run manifest
(commit \texttt{b8d40a1}, tag \texttt{simula-v1.2-worked-example},
seed \texttt{20251115}) is pinned both in the \texttt{moscow.yaml}
\texttt{reproducibility\_block} and in the canonical fixture
\path{docs/schema/tests/fixtures/simula/config.sample.json}; they are
locked by the pytest
\texttt{test\_simula\_config\_has\_reproducibility\_manifest}.

\begin{lstlisting}[language=yaml,caption={Excerpt from
\texttt{docs/schema/common/moscow.yaml} for the Simula
domain.},label=lst:sim-moscow-yaml]
simula:
  - { id: M01, priority: Must,   title: "Extraction pipeline",     ticket: SIM-ENG-001 }
  - { id: M05, priority: Must,   title: "HANA spatial policy",     ticket: SIM-HANA-SRID }
  - { id: M06, priority: Must,   title: "Reference run (M06)",     ticket: SIM-REF-RUN,
      reproducibility_block: { commit: "b8d40a1",
...
\noindent
Any change to the milestone list in this roadmap must be reflected in
