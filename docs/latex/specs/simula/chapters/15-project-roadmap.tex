...
\mermaidfig[\textwidth]{diagrams/dependency-graph}%
  {Simula dependency graph. Amber nodes are Must (critical path); blue are
    Should (consumer-corpus allowance); dotted green are Could (discretionary).
    Solid arrows are hard prerequisites; dashed arrows indicate conditional
    starts (the target is unlocked when the upstream completes \emph{and} the
    v1.3 trigger in \cref{sec:sim-v13-trigger} fires).}%
  {fig:sim-dependency-graph}
