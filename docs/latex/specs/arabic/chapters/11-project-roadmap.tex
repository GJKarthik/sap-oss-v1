...
M01 & ---       & M & AP-ENG-001         & OCR pipeline (Tesseract + Arabic model) passes the fixtures in \cref{ch:ap-testing}. \\
M02 & M01       & M & AP-ENG-002         & VAT engine unit tests pass (\cref{ch:vat-engine}); country-configurable rates.\\
...
M07 & M06       & M & AP-PARALLEL-001    & Beta parallel-run gate passes (\cref{sec:ap-parallel-run}). \\
...
Alpha & Engineering only, DEV + UAT, fixture documents only. & M01, M02, M03 closed; all acceptance criteria in \cref{ch:ap-testing} pass on the pilot fixture; reproducibility of OCR and VAT outputs confirmed on two consecutive runs. \\
...
vLLM hosting (\texttt{vllm-only} policy) & Shared platform & M02 & Graceful-degradation ladder (\cref{sec:degradation-ladder} of the Regulations spec) provides fallback; slip tolerable. \\
...
\begin{itemize}
  \item \textbf{Mandatory refresh}: at the first cycle after GA, and
        ...
        volume assumptions, unit economics (\cref{sec:ap-unit-economics}),
...
\section{Machine-readable milestone registry}\label{sec:ap-moscow-yaml}
...
\texttt{docs/schema/common/moscow.yaml} for the Arabic AP
domain.},label=lst:ap-moscow-yaml]
arabic:
  - { id: M01, priority: Must,   title: "OCR pipeline",       ticket: AP-ENG-001 }
  - { id: M02, priority: Must,   title: "VAT engine",         ticket: AP-ENG-002 }
  - { id: M03, priority: Must,   title: "Pilot prep (200 docs)", ticket: AP-PILOT-PREP }
  - { id: M04, priority: Must,   title: "Identity wiring",    ticket: REG-INTG-AP-001 }
  - { id: S01, priority: Should, title: "Egypt onboarded",    ticket: AP-WAVE-002 }
  # ...
\end{lstlisting}

\noindent
Any change to the milestone list in this chapter must be reflected in
\path{docs/schema/common/moscow.yaml} in the same pull request; the CI
workflow \path{.github/workflows/schema-validation.yaml} fails closed
if the two drift. The
\path{docs/schema/tests/integration/test_roadmap_references_resolve}
pytest guards the invariant locally via
\path{docs/schema/.pre-commit-config.yaml}.
