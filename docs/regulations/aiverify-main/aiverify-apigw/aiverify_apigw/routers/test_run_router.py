...
async def run_test(input_data: TestRunInput, session: Session = Depends(get_db_session)) -> TestRunOutput:
    ...
    try:
        return await _run_test(input_data, session)
...
async def _run_test(input_data: TestRunInput, session: Session) -> TestRunOutput:
...
def update_test_run_status(
    ...
    try:
        ...
        session.rollback()
        logger.error(f"Error updating test run {test_run_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
def cancel_test_run(test_run_id: str, session: Session = Depends(get_db_session)) -> TestRunOutput:
    try:
        ...
        session.rollback()
        logger.error(f"Error cancelling test run {test_run_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
def delete_test_run(test_run_id: str, session: Session = Depends(get_db_session)) -> None:
    try:
        ...
        session.rollback()
        logger.error(f"Error deleting test run {test_run_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
