...
const classNameForThemingService = (packageName: 'fiori' | 'main' | 'ai') => `Ui5Webcomponents${pascalCase(packageName)}ThemingService`;

class ThemingServiceFile extends AngularGeneratedFile {
  className: string;
  override apfPath = `@ui5/webcomponents-ngx/${this.baseDir}`;

  constructor(protected baseDir: string, protected packageName: 'fiori' | 'main' | 'ai') {
    super();
...
const packageNames = ['fiori', 'main', 'ai'];
...
const componentGenerator = (components: ComponentData[]) =>
  ui5componentsWrapper(components, {
    ...
    exportFileNameFactory: (path) => {
      ...
      const finalPath = path.replace(/^@ui5\/webcomponents(-base|-fiori|-ai)?\/dist\/(.*)\.js$/, '$2').split('/');
    ...
    apfPathFactory: (path) => {
      ...
      const module = path.match(/^@ui5\/webcomponents-(.*)\/dist\/(.*)\.js$/)?.[1];
      const finalPath = path.replace(/^@ui5\/webcomponents(-base|-fiori|-ai)?\/dist\/(.*)(index)?\.(js|ts)$/, '$2').split('/');
  ...
  transformers: [
    (components: ComponentData[]) => {
      ...
      for (const packageName of packageNames) {
        const themingServiceFile = new ThemingServiceFile(`${packageName}/theming`, packageName as 'fiori' | 'main' | 'ai');
