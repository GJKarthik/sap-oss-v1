...
const releaseHotfix = async () => {
  ...
  if (gitDirty !== '') {
    throw new Error('Cannot release a hotfix from a dirty git tree, please commit your changes first:' + '\n' + gitDirty);
  }
...
releaseHotfix().then(() => {
  execAndLog(`git push --set-upstream origin ${hotfixBranchName}`)
