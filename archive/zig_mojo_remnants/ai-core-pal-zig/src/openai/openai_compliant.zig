...
pub fn buildChatResponse(
    ...
    return try std.json.stringifyAlloc(allocator, resp, .{});
}
...
pub fn buildModelsResponse(allocator: Allocator) ![]u8 {
    const payload = .{
        .object = "list",
        .data = [_]struct {
            id: []const u8,
            ...
            .{ .id = "mcppal-mesh-gateway-v1", .object = "model", .created = 0, .owned_by = "ainuc" },
            .{ .id = "mcppal-pal-catalog-v1", .object = "model", .created = 0, .owned_by = "ainuc" },
        },
    };
    return try std.json.stringifyAlloc(allocator, payload, .{});
}

pub fn getJsonStr(value: std.json.Value, key: []const u8) ?[]const u8 {
    if (value != .object) return null;
    const node = value.object.get(key) orelse return null;
    return switch (node) {
        .string => |s| s,
        else => null,
    };
