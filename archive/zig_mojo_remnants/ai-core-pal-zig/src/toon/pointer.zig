...
pub const ToonPointer = struct {
    ...
    pub fn hanaTable(
        ...
        if (filter) |f| {
            try query_buf.appendSlice("$filter=");
            try query_buf.appendSlice(f);
        }
        if (select_columns) |cols| {
            if (query_buf.items.len > 0) try query_buf.append('&');
            try query_buf.appendSlice("$select=");
            try query_buf.appendSlice(cols);
        }
        
        return ToonPointer{
            .ptr_type = .hana_table,
            .location = location,
            .query = if (query_buf.items.len > 0) try query_buf.toOwnedSlice() else null,
    ...
    pub fn hanaVector(
        ...
        var query_buf = std.ArrayList(u8).init(allocator);
        try query_buf.writer().print("k={d}", .{k});
        if (query_vector_ref) |ref| {
            try query_buf.appendSlice("&query_ref=");
            try query_buf.appendSlice(ref);
        }
        
        return ToonPointer{
            .ptr_type = .hana_vector,
            .location = location,
            .query = try query_buf.toOwnedSlice(),
            .credentials_ref = credentials,
            .format = .auto,
            .columns = null,
            .ttl_seconds = 3600,
    ...
    pub fn hanaGraph(
        ...
        var location_buf = std.ArrayList(u8).init(allocator);
        try location_buf.writer().print("{s}.{s}", .{ schema, workspace });
        if (vertex_type) |vt| {
            try location_buf.append('/');
            try location_buf.appendSlice(vt);
        }
        ...
        return ToonPointer{
            .ptr_type = .hana_graph,
            .location = try location_buf.toOwnedSlice(),
            .query = query,
            .credentials_ref = credentials,
            .format = .auto,
            .columns = null,
    ...
    pub fn sapObject(
        ...
        var query_buf = std.ArrayList(u8).init(allocator);
        if (format != .auto) {
            try query_buf.writer().print("format={s}", .{@tagName(format)});
        }
        if (columns) |cols| {
            if (query_buf.items.len > 0) try query_buf.append('&');
            try query_buf.appendSlice("columns=");
            try query_buf.appendSlice(cols);
        }
...
/// Initialize the global revocation registry
...
/// Check if a pointer is valid (not expired AND not revoked)
