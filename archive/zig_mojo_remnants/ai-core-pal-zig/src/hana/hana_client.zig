...
pub const HanaClient = struct {
    ...
    pub fn executeSQL(self: *HanaClient, sql: []const u8) ![]const u8 {
        ...
        try bw.writeAll("\"}");
        const req_body = try body_buf.toOwnedSlice();
        ...
        _ = std.posix.write(sock, req.items) catch |err| {
        ...
        if (sep > 0) {
            ...
            return try self.allocator.dupe(u8, body_slice);
        }
    ...
    pub fn discoverSchema(self: *HanaClient, schema_name: []const u8, db: *schema_mod.Database) !void {
        ...
        var table_names = std.ArrayList([]const u8).init(self.allocator);
        ...
        for (table_names.items) |table_name| {
            var table_schema = schema_mod.TableSchema{
                .name = table_name,
                .columns = std.ArrayList(schema_mod.Column).init(self.allocator),
                .foreign_keys = std.ArrayList(schema_mod.ForeignKey).init(self.allocator),
                .primary_key = &.{},
            };
...
fn parsePrimaryKeys(allocator: Allocator, json: []const u8, table: *schema_mod.TableSchema) !void {
    var pk_names = std.ArrayList([]const u8).init(allocator);
    var pos: usize = 0;
    while (pos < json.len) {
        ...
        try pk_names.append(val);
        pos = key_pos + key.len;
    }
    if (pk_names.items.len > 0) {
        table.primary_key = try pk_names.toOwnedSlice();
    }
}
