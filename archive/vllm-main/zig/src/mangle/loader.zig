...
pub const MangleLoader = struct {
    ...
    /// Standard subdirectories to load from (in order)
    const subdirs = [_][]const u8{ "standard", "a2a", "toon", "domain" };

    /// Load all .mg files from the mangle directory and subdirectories
    pub fn loadAll(self: *MangleLoader) !usize {
        var count: usize = 0;

        // Load from subdirectories in order: standard, a2a, toon, domain
        for (subdirs) |subdir| {
            count += try self.loadFromSubdir(subdir);
        }

        // Also load any .mg files in the root mangle/ directory (legacy support)
        count += try self.loadFromDir(self.mangle_dir, "");

        return count;
    }

    /// Load .mg files from a subdirectory
    fn loadFromSubdir(self: *MangleLoader, subdir: []const u8) !usize {
        var path_buf: [512]u8 = undefined;
        const full_path = try std.fmt.bufPrint(&path_buf, "{s}/{s}", .{ self.mangle_dir, subdir });
        return self.loadFromDir(full_path, subdir);
    }

    /// Load .mg files from a specific directory
    fn loadFromDir(self: *MangleLoader, dir_path: []const u8, prefix: []const u8) !usize {
        var dir = fs.cwd().openDir(dir_path, .{ .iterate = true }) catch |err| {
            // Subdirectory may not exist - that's OK
            if (err == error.FileNotFound) return 0;
            std.log.warn("Could not open mangle directory '{s}': {}", .{ dir_path, err });
            return 0;
        };
        defer dir.close();
        ...
        while (try iter.next()) |entry| {
            ...
            // Build filename with prefix (e.g., "standard/facts.mg")
            var name_buf: [256]u8 = undefined;
            const filename = if (prefix.len > 0)
                try std.fmt.bufPrint(&name_buf, "{s}/{s}", .{ prefix, entry.name })
            else
                entry.name;
            ...
            std.log.info("Loaded mangle file: {s} ({d} bytes)", .{ filename, stat.size });
            count += 1;
        }
