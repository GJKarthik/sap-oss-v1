...
pub const GpuMemoryPool = struct {
    ...
    pub fn transferToGpu(self: *GpuMemoryPool, slot: *BufferSlot) !void {
        ...
        // GPU H2D transfer dispatch
        // On Apple Silicon unified memory, MTLBuffer.contents() IS the CPU pointer —
        // no copy is needed. The slot's data is already GPU-visible at this point.
        // On CUDA (Linux), a real cuMemcpyHtoD would go here under the -Dgpu flag.
        if (self.gpu_ctx) |ctx| _ = ctx; // suppress unused warning
    ...
    pub fn transferFromGpu(self: *GpuMemoryPool, slot: *BufferSlot) !void {
        ...
        // GPU D2H transfer dispatch
        // On Apple Silicon unified memory, results are already in CPU-visible memory —
        // no copy is needed. On CUDA (Linux), cuMemcpyDtoH would go here.
        if (self.gpu_ctx) |ctx| _ = ctx; // suppress unused warning
