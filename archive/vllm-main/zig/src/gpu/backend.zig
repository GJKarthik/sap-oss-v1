...
// ============================================================================
// CUDA Backend — routes through cuda_backend.zig for real CUDA dispatch
// ============================================================================

pub const CudaBackend = struct {
    allocator: Allocator,
    inner: *cuda_backend.CudaBackend,

    // Statistics (mirrors inner backend counters for unified API)
    ...
    pub fn init(allocator: Allocator) !*CudaBackend {
        ...
        backend.* = .{
            .allocator = allocator,
            .inner = try cuda_backend.CudaBackend.init(allocator, .{}),
    ...
    pub fn submitBatch(self: *CudaBackend, batch: *const Batch) !BatchResult {
        ...
        // Route through real cuda_backend embeddings (CUDA C kernel or CPU fallback)
        const f32_output = std.mem.bytesAsSlice(f32, output);
        const tokens = std.mem.bytesAsSlice(u32, batch.input_data);
        const n_tokens = @min(tokens.len, batch.batch_size);
        _ = try self.inner.embeddings(tokens[0..n_tokens], f32_output, batch.embedding_dim);

        const output_bytes = output_size;
        _ = self.total_dispatches.fetchAdd(1, .monotonic);
        _ = self.total_bytes.fetchAdd(output_bytes, .monotonic);
