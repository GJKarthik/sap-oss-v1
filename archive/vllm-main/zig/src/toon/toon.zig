...
pub const Parser = struct {
    ...
    fn parsePipeArray(self: *Parser, raw: []const u8) !Value {
        ...
        return Value{ .array_val = try items.toOwnedSlice() };
    }
    ...
    fn parseQuotedString(self: *Parser) !Value {
        ...
        return Value{ .string_val = unescaped };
    }

    /// Process escape sequences in a string: \n \t \r \\ \" \' → actual chars
    fn unescapeString(allocator: Allocator, raw: []const u8) ![]const u8 {
        // Fast path: no backslashes → just dupe
...
pub const Writer = struct {
    ...
    fn writeValue(self: *Writer, value: Value) !void {
        switch (value) {
            .null_val => try self.buffer.appendSlice("~"),
            .bool_val => |b| try self.buffer.appendSlice(if (b) "true" else "false"),
            .int_val => |i| {
            ...
            .array_val => |arr| {
                ...
                if (arr.len > 0 and isSimpleArray(arr)) {
                    for (arr, 0..) |item, idx| {
                        if (idx > 0) try self.buffer.append('|');
                        try self.writeValue(item);
                    }
                } else {
                    try self.buffer.append('[');
                    for (arr, 0..) |item, idx| {
                        if (idx > 0) try self.buffer.appendSlice(", ");
                        try self.writeValue(item);
                    }
                    try self.buffer.append(']');
                }
            ...
            .object_val => |map| {
                ...
                while (iter.next()) |entry| {
                    ...
                    first = false;
                    try self.buffer.appendSlice(entry.key_ptr.*);
                    try self.buffer.append(':');
...
test "parse tilde null value" {
    ...
    try std.testing.expect(value == .object_val);
    try std.testing.expect(value.object_val.get("name").? == .null_val);
    try std.testing.expectEqual(@as(i64, 30), value.object_val.get("age").?.int_val);
}

test "parse explicit array syntax" {
    const allocator = std.testing.allocator;
    ...
    const items = value.object_val.get("items").?.array_val;
    try std.testing.expectEqual(@as(usize, 3), items.len);
    try std.testing.expectEqualStrings("a", items[0].string_val);
    try std.testing.expectEqualStrings("b", items[1].string_val);
    try std.testing.expectEqualStrings("c", items[2].string_val);
}
...
test "parse nested object" {
    ...
    const user = value.object_val.get("user").?.object_val;
    try std.testing.expectEqualStrings("John", user.get("name").?.string_val);
    try std.testing.expectEqual(@as(i64, 30), user.get("age").?.int_val);
}

test "parse null variants" {
    const allocator = std.testing.allocator;
    const input = "a:~ b:null c:nil";

    var parser = Parser.init(allocator, input);
    var value = try parser.parse();
    defer value.deinit();

    try std.testing.expect(value.object_val.get("a").? == .null_val);
    try std.testing.expect(value.object_val.get("b").? == .null_val);
    try std.testing.expect(value.object_val.get("c").? == .null_val);
}

test "parse boolean variants" {
    const allocator = std.testing.allocator;
    const input = "a:true b:false c:yes d:no e:Y f:N";

    var parser = Parser.init(allocator, input);
    var value = try parser.parse();
    defer value.deinit();

    try std.testing.expectEqual(true, value.object_val.get("a").?.bool_val);
    try std.testing.expectEqual(false, value.object_val.get("b").?.bool_val);
    try std.testing.expectEqual(true, value.object_val.get("c").?.bool_val);
    try std.testing.expectEqual(false, value.object_val.get("d").?.bool_val);
    try std.testing.expectEqual(true, value.object_val.get("e").?.bool_val);
    try std.testing.expectEqual(false, value.object_val.get("f").?.bool_val);
}

test "parse float value" {
    const allocator = std.testing.allocator;
    const input = "price:9.99 tax:0.08";

    var parser = Parser.init(allocator, input);
    var value = try parser.parse();
    defer value.deinit();

    const price = value.object_val.get("price").?.float_val;
    try std.testing.expect(price > 9.98 and price < 10.0);
    const tax = value.object_val.get("tax").?.float_val;
    try std.testing.expect(tax > 0.07 and tax < 0.09);
}

test "parse quoted string value" {
    const allocator = std.testing.allocator;
    const input = "msg:\"hello world\" count:5";

    var parser = Parser.init(allocator, input);
    var value = try parser.parse();
    defer value.deinit();

    try std.testing.expectEqualStrings("hello world", value.object_val.get("msg").?.string_val);
    try std.testing.expectEqual(@as(i64, 5), value.object_val.get("count").?.int_val);
}
...
test "parse comment lines" {
    ...
    try std.testing.expectEqualStrings("John", value.object_val.get("name").?.string_val);
    try std.testing.expectEqual(@as(i64, 30), value.object_val.get("age").?.int_val);
}
...
test "parse semicolon separator" {
    ...
    try std.testing.expectEqualStrings("John", value.object_val.get("name").?.string_val);
    try std.testing.expectEqual(@as(i64, 30), value.object_val.get("age").?.int_val);
    try std.testing.expectEqual(true, value.object_val.get("active").?.bool_val);
}
...
test "parse single value" {
    ...
    try std.testing.expectEqualStrings("ok", value.object_val.get("status").?.string_val);
}

test "fromJson nested json" {
    const allocator = std.testing.allocator;
    const json =
        \\{"user": {"name": "John", "age": 30}, "scores": [100, 95]}
    ;
...
test "toon token savings reasonable" {
    const allocator = std.testing.allocator;
    const json =
    ...
    const savings = try tokenSavings(allocator, json);
    // Savings should be between 10% and 80%
    try std.testing.expect(savings.savings_pct > 10.0);
    try std.testing.expect(savings.savings_pct < 80.0);
    try std.testing.expect(savings.toon_tokens < savings.json_tokens);
}

test "parse escape sequences in quoted strings" {
    const allocator = std.testing.allocator;
    const input = "msg:\"hello\\nworld\" tab:\"col1\\tcol2\" escaped:\"a\\\\b\" quote:\"say \\\"hi\\\"\"";
    ...
    try std.testing.expectEqualStrings("hello\nworld", value.object_val.get("msg").?.string_val);
    // \t should become actual tab
    try std.testing.expectEqualStrings("col1\tcol2", value.object_val.get("tab").?.string_val);
    // \\ should become single backslash
    try std.testing.expectEqualStrings("a\\b", value.object_val.get("escaped").?.string_val);
    // \" should become actual quote
    try std.testing.expectEqualStrings("say \"hi\"", value.object_val.get("quote").?.string_val);
}