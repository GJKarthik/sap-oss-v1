...
pub const BatchScheduler = struct {
    ...
    pub fn deinit(self: *Self) void {
        // Free any pending requests that were never batched
    ...
    pub fn submit(self: *Self, tokens: []const u32, max_new_tokens: usize) !u64 {
        // Atomically allocate request ID (no lock needed for ID generation)
    ...
    pub fn releaseBatch(self: *Self) void {
        ...
        if (self.active_batch) |batch| {
            // Free individual Request objects that were moved from pending
