...
# Default timeout constant for validation
alias DEFAULT_TIMEOUT_SECONDS: Int = 30
alias MIN_TIMEOUT_SECONDS: Int = 1
alias MAX_TIMEOUT_SECONDS: Int = 300
...
struct AICoreConfig:
    ...
    fn __init__(
        ...
        self.token_ttl_seconds = token_ttl_seconds if token_ttl_seconds > 0 else 3300
        # Validate timeout is within reasonable bounds
        if request_timeout_seconds < MIN_TIMEOUT_SECONDS:
            self.request_timeout_seconds = DEFAULT_TIMEOUT_SECONDS
        elif request_timeout_seconds > MAX_TIMEOUT_SECONDS:
            self.request_timeout_seconds = MAX_TIMEOUT_SECONDS
        else:
...
struct AICoreAdapter:
    ...
    fn _get_auth_token(inout self) -> String:
        ...
        Caches the token and automatically refreshes when the TTL expires.
        Default TTL is 3300s (55 min), conservative for typical 1h XSUAA tokens.
        
        Note: Token refresh is not fully thread-safe. In concurrent scenarios,
        multiple threads may attempt refresh simultaneously. This is acceptable
        as duplicate refreshes are idempotent, just slightly wasteful.
        """
        # Check if cached token is still valid
        # Note: This check is not atomic - concurrent calls may both see expired
        # and attempt refresh. This is acceptable (idempotent, just wasteful).
        if self.auth_token != "":
            try:
                var time_mod = Python.import_module("time")
                var now = Float64(time_mod.time())
                var age = now - self._token_fetched_at
                if age < Float64(self.config.token_ttl_seconds):
        ...
        # Use Python requests for token fetch
        try:
            var time_mod = Python.import_module("time")
            var requests = Python.import_module("requests")
            # Ensure timeout is valid before making request
            var timeout_val = self.config.request_timeout_seconds
            if timeout_val <= 0:
...
- Only output the requested fields, nothing else."""
        ...
        try:
            ...
            if response.status_code == 401:
                # Token rejected — invalidate and signal retry
            ...
            # Provide fallback error message in case str(e) fails
    ...
    fn complete_with_retry(
        ...
        while retries < max_retries:
            ...
            if retries < max_retries:
                try:
                    var time_mod = Python.import_module("time")
                    var delay = Float64(1 << (retries - 1))  # 1, 2, 4, ...
    ...
    fn stream_complete(
        ...
        Opens the request with `stream=True` and accumulates `data:` lines
        until the server sends `[DONE]`.  Each SSE chunk carries a delta
        token in `choices[0].delta.content`.  The accumulated text is then
        cleaned as TOON before returning.

        Falls back to `complete()` on any streaming error.
        """
        var token = self._get_auth_token()
        if token == "":
        ...
        try:
            ...
            var payload = self._build_payload(
                system_prompt if system_prompt != "" else "Respond in TOON format.",
                prompt,
            )
            payload["stream"] = True
            ...
            # Accumulate streamed delta tokens
            var accumulated = String()
            for line in response.iter_lines():
                var text = str(line)
                if not text.startswith("data:"):
                    continue
                var data_str = text[len("data:"):]
                data_str = data_str.strip()
                if data_str == "[DONE]":
                    break
                try:
                    var chunk = json_mod.loads(data_str)
                    var delta = str(chunk["choices"][0]["delta"].get("content", ""))
                    accumulated += delta
                except:
                    continue
