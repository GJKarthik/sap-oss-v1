...
const c_api_version = @import("../c_api/version.zig");
...
pub export fn kuzu_get_version() [*c]const u8 {
    const c_str = c_api_version.getVersionCString() orelse return null;
    return @ptrCast(c_str);
}

pub export fn kuzu_get_storage_version() u64 {
    return c_api_version.getStorageVersion();
}
...
test "version api" {
    const version = kuzu_get_version();
