//! binder/ddl/property_definition parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/ddl/property_definition";
}

test "property_definition binding path" {
    try std.testing.expectEqualStrings("binder/ddl/property_definition", bindingPath());
}

test "property_definition effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/ddl/property_definition"));
}
