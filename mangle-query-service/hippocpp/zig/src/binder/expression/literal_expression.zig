//! binder/expression/literal_expression parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/expression/literal_expression";
}

test "literal_expression binding path" {
    try std.testing.expectEqualStrings("binder/expression/literal_expression", bindingPath());
}

test "literal_expression effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/expression/literal_expression"));
}
