//! binder/query/query_graph parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/query/query_graph";
}

test "query_graph binding path" {
    try std.testing.expectEqualStrings("binder/query/query_graph", bindingPath());
}

test "query_graph effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/query/query_graph"));
}
