//! binder/bind/bind_ddl parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_ddl";
}

test "bind_ddl binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_ddl", bindingPath());
}

test "bind_ddl effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_ddl"));
}
