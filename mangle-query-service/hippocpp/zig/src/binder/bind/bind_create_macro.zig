//! binder/bind/bind_create_macro parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_create_macro";
}

test "bind_create_macro binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_create_macro", bindingPath());
}

test "bind_create_macro effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_create_macro"));
}
