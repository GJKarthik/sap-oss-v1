...
pub const QueryResult = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            .allocator = allocator,
            .columns = std.ArrayList(ColumnInfo).init(allocator),
            .chunks = std.ArrayList(DataChunk).init(allocator),
            .num_tuples = 0,
            .execution_time_ns = 0,
            .success = true,
            .error_message = null,
            .warning_messages = std.ArrayList([]const u8).init(allocator),
            .query_id = 0,
            .is_exhausted = false,
            .profiling_enabled = false,
            .operator_profiles = std.ArrayList(OperatorProfile).init(allocator),
        };
    }
    ...
    pub fn addWarning(self: *Self, message: []const u8) !void {
        try self.warning_messages.append(message);
    }
...
pub const OperatorProfile = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, id: u32, op_type: PhysicalOperatorType) OperatorProfile {
        return .{
            .operator_id = id,
...
pub const ResultSetDescriptor = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            ...
            .column_types = std.ArrayList(common.LogicalType).init(allocator),
        };
    }
...
pub const ResultSet = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            ...
            .data_chunks = std.ArrayList(DataChunk).init(allocator),
            .multiplicity = 1,
        };
    }
...
pub const ProgressBar = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            .allocator = allocator,
            .enabled = false,
            .pipelines = std.AutoHashMap(u64, PipelineProgress).init(allocator),
        };
    }
...
pub const ProcessorTask = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, sink: *PhysicalOperator, context: *ExecutionContext, result: *QueryResult) Self {
        return .{
            ...
            .context = context,
            .result = result,
            .children = std.ArrayList(*ProcessorTask).init(allocator),
            .status = .PENDING,
    ...
    pub fn execute(self: *Self) !void {
        ...
        while (!self.context.isInterrupted() and !self.context.checkTimeout()) {
            ...
            if (chunk.num_tuples > 0) {
                ...
                try self.result.chunks.append(stored_chunk);
            }
...
pub const TaskScheduler = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, num_threads: u32) Self {
        return .{
            ...
            .pending_tasks = std.ArrayList(*ProcessorTask).init(allocator),
            .running_tasks = std.ArrayList(*ProcessorTask).init(allocator),
        };
    }
...
pub const QueryProcessor = struct {
    ...
    fn collectProfilingInfo(self: *Self, result: *QueryResult, op: *PhysicalOperator) !void {
        ...
        for (op.children.items) |child| {
            try profile.children.append(child.operator_id);
            try self.collectProfilingInfo(result, child);
        }
        
        try result.operator_profiles.append(profile);
    }
...
pub const ResultCollector = struct {
    ...
    pub fn create(allocator: std.mem.Allocator) !*Self {
        ...
        self.* = .{
            ...
            .result_chunks = std.ArrayList(DataChunk).init(allocator),
            .total_tuples = 0,
            .materialized = false,
        };
        return self;
    ...
    fn collectGetNext(base: *PhysicalOperator, chunk: *DataChunk) !ResultState {
        ...
        if (result == .HAS_MORE_TUPLES and chunk.num_tuples > 0) {
...
pub const ProjectionOperator = struct {
    ...
    pub fn create(allocator: std.mem.Allocator) !*Self {
        ...
        self.* = .{
            ...
            .projection_cols = std.ArrayList(u32).init(allocator),
        };
        return self;
    }
    
    pub fn destroy(self: *Self) void {
        self.projection_cols.deinit();
        self.base.deinit();
        self.base.allocator.destroy(self);
    }
    
    pub fn addProjectionColumn(self: *Self, col_idx: u32) !void {
        try self.projection_cols.append(col_idx);
    }
