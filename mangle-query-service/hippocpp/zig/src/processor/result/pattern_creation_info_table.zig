...
pub const PatternCreationInfoTable = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) PatternCreationInfoTable {
        return .{
            .allocator = allocator,
            .rows = std.ArrayList(PatternCreationInfo).init(allocator),
        };
    }
...
test "pattern creation totals" {
    ...
    try table.append(.{ .pattern_name = "p1", .created_nodes = 2, .created_rels = 1 });
    try table.append(.{ .pattern_name = "p2", .created_nodes = 3, .created_rels = 0 });
