...
pub const ResultVector = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, data_type: LogicalType, capacity: usize) !Self {
        ...
        var null_mask = std.ArrayList(bool).init(allocator);
    ...
    pub fn append(self: *Self, value: Value) !void {
        ...
        try self.null_mask.append(value.isNull());
        self.size += 1;
    }
...
test "result vector" {
    ...
    try vec.append(Value.int64(1));
    try vec.append(Value.int64(2));
    try vec.append(Value.int64(3));
