//! catalog/catalog_entry/catalog_entry parity module.
...
pub fn modulePath() []const u8 {
    return "catalog/catalog_entry/catalog_entry";
}

test "catalog_entry module path" {
    try std.testing.expectEqualStrings("catalog/catalog_entry/catalog_entry", modulePath());
}

test "catalog_entry resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("catalog/catalog_entry/catalog_entry"));
}
