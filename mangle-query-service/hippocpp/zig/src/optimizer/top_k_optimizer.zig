//! optimizer/top_k_optimizer parity module.
...
pub fn modulePath() []const u8 {
    return "optimizer/top_k_optimizer";
}

test "top_k_optimizer module path" {
    try std.testing.expectEqualStrings("optimizer/top_k_optimizer", modulePath());
}

test "top_k_optimizer resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("optimizer/top_k_optimizer"));
}
