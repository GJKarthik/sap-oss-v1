...
const logical_plan = @import("../planner/logical_plan.zig");
const physical_operator = @import("../processor/physical_operator.zig");
const expression_mod = @import("../expression/expression.zig");
...
pub const PredicateSet = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            .allocator = allocator,
            .equality_predicates = std.ArrayList(*Expression).init(allocator),
            .non_equality_predicates = std.ArrayList(*Expression).init(allocator),
        };
    }
    
    pub fn deinit(self: *Self) void {
        self.equality_predicates.deinit();
        self.non_equality_predicates.deinit();
    }
    
    /// Add a predicate to the set
    pub fn addPredicate(self: *Self, predicate: *Expression) !void {
        if (predicate.expr_type == .EQUALS) {
            try self.equality_predicates.append(predicate);
        } else {
            try self.non_equality_predicates.append(predicate);
        }
    }
...
pub const ColumnPredicateSet = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            .allocator = allocator,
            .predicates = std.ArrayList(ColumnPredicate).init(allocator),
        };
    }
...
pub const FilterPushDownOptimizer = struct {
    ...
    fn visitFilter(self: *Self, op: *LogicalOperator) !*LogicalOperator {
        ...
        if (op.filter_expression) |pred| {
            ...
            if (!self.isLiteralTrue(pred)) {
                try self.predicate_set.addPredicate(pred);
            }
        }
    ...
    fn visitCrossProduct(self: *Self, op: *LogicalOperator) !*LogicalOperator {
        ...
        var probe_preds = PredicateSet.init(self.allocator);
        defer probe_preds.deinit();
        
        var build_preds = PredicateSet.init(self.allocator);
        defer build_preds.deinit();
        
        // Partition predicates by which side they can be pushed to
        var all_preds = std.ArrayList(*Expression).init(self.allocator);
        defer all_preds.deinit();
        try self.predicate_set.getAllPredicates(&all_preds);
        
        for (all_preds.items) |pred| {
            const in_probe = self.canEvaluateOn(pred, op.getChild(0));
            const in_build = self.canEvaluateOn(pred, op.getChild(1));
            
            if (in_probe and !in_build) {
                try probe_preds.addPredicate(pred);
            } else if (!in_probe and in_build) {
                try build_preds.addPredicate(pred);
            } else {
                try remaining_preds.addPredicate(pred);
            }
        }
        
        // Push predicates to children
        if (op.getChild(0)) |probe_child| {
            var probe_optimizer = FilterPushDownOptimizer.init(self.allocator, self.config);
            probe_optimizer.predicate_set = probe_preds;
            const new_probe = try probe_optimizer.visitOperator(probe_child);
            op.children.items[0] = new_probe;
        }
        
        if (op.getChild(1)) |build_child| {
            var build_optimizer = FilterPushDownOptimizer.init(self.allocator, self.config);
            build_optimizer.predicate_set = build_preds;
            const new_build = try build_optimizer.visitOperator(build_child);
            op.children.items[1] = new_build;
        }
        ...
        var remaining_all = std.ArrayList(*Expression).init(self.allocator);
        defer remaining_all.deinit();
    ...
    fn tryConvertToHashJoin(self: *Self, op: *LogicalOperator, remaining: *PredicateSet) !?*LogicalOperator {
        ...
        var join_conditions = std.ArrayList(JoinCondition).init(self.allocator);
        defer join_conditions.deinit();
        
        var non_join_preds = std.ArrayList(*Expression).init(self.allocator);
        defer non_join_preds.deinit();
        
        for (remaining.equality_predicates.items) |pred| {
            if (try self.tryExtractJoinCondition(pred, op)) |jc| {
                try join_conditions.append(jc);
            } else {
                try non_join_preds.append(pred);
            }
        }
        ...
        for (non_join_preds.items) |p| {
            try remaining.equality_predicates.append(p);
        }
