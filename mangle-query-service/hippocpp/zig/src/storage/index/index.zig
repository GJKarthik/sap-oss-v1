//! storage/index/index parity module.
...
test "index module path" {
    try std.testing.expectEqualStrings("storage/index/index", modulePath());
}

test "index resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/index/index"));
}
