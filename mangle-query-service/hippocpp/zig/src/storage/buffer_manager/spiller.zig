//! storage/buffer_manager/spiller parity module.
...
pub fn modulePath() []const u8 {
    return "storage/buffer_manager/spiller";
}

test "spiller module path" {
    try std.testing.expectEqualStrings("storage/buffer_manager/spiller", modulePath());
}

test "spiller resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/buffer_manager/spiller"));
}
