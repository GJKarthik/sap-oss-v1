//! storage/table/list_chunk_data parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/list_chunk_data";
}

test "list_chunk_data module path" {
    try std.testing.expectEqualStrings("storage/table/list_chunk_data", modulePath());
}

test "list_chunk_data resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/list_chunk_data"));
}
