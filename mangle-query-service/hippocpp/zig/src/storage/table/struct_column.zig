//! storage/table/struct_column parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/struct_column";
}

test "struct_column module path" {
    try std.testing.expectEqualStrings("storage/table/struct_column", modulePath());
}

test "struct_column resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/struct_column"));
}
