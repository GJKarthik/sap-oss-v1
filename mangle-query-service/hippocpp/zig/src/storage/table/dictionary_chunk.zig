//! storage/table/dictionary_chunk parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/dictionary_chunk";
}

test "dictionary_chunk module path" {
    try std.testing.expectEqualStrings("storage/table/dictionary_chunk", modulePath());
}

test "dictionary_chunk resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/dictionary_chunk"));
}
