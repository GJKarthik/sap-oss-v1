//! storage/table/column_chunk_data parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/column_chunk_data";
}

test "column_chunk_data module path" {
    try std.testing.expectEqualStrings("storage/table/column_chunk_data", modulePath());
}

test "column_chunk_data resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/column_chunk_data"));
}
