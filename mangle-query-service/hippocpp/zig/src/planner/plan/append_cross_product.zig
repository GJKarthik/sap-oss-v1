//! planner/plan/append_cross_product parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_cross_product";
}

test "append_cross_product planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_cross_product", plannerPath());
}

test "append_cross_product planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_cross_product"));
}
