//! planner/plan/plan_node_semi_mask parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/plan_node_semi_mask";
}

test "plan_node_semi_mask planner path" {
    try std.testing.expectEqualStrings("planner/plan/plan_node_semi_mask", plannerPath());
}

test "plan_node_semi_mask planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/plan_node_semi_mask"));
}
