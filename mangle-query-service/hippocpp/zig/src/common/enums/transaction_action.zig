//! common/enums/transaction_action parity module.
...
pub fn modulePath() []const u8 {
    return "common/enums/transaction_action";
}

test "transaction_action module path" {
    try std.testing.expectEqualStrings("common/enums/transaction_action", modulePath());
}

test "transaction_action resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/enums/transaction_action"));
}
