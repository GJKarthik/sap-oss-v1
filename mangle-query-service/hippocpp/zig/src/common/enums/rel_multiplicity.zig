//! common/enums/rel_multiplicity parity module.
...
pub fn modulePath() []const u8 {
    return "common/enums/rel_multiplicity";
}

test "rel_multiplicity module path" {
    try std.testing.expectEqualStrings("common/enums/rel_multiplicity", modulePath());
}

test "rel_multiplicity resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/enums/rel_multiplicity"));
}
