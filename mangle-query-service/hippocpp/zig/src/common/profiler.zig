...
pub const OperatorMetrics = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, name: []const u8) OperatorMetrics {
        return .{
            .name = name,
            .children = std.ArrayList(*OperatorMetrics).init(allocator),
        };
    }
...
pub const QueryProfiler = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) QueryProfiler {
        return .{
            .allocator = allocator,
            .operators = std.ArrayList(OperatorMetrics).init(allocator),
        };
    }
    ...
    pub fn addOperator(self: *QueryProfiler, name: []const u8) !*OperatorMetrics {
        var op = OperatorMetrics.init(self.allocator, name);
