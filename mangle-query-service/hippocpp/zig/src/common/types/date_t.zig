//! common/types/date_t parity module.
...
pub fn modulePath() []const u8 {
    return "common/types/date_t";
}

test "date_t module path" {
    try std.testing.expectEqualStrings("common/types/date_t", modulePath());
}

test "date_t resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/types/date_t"));
}
