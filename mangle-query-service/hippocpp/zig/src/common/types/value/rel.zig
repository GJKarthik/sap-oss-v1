//! common/types/value/rel parity module.
...
pub fn modulePath() []const u8 {
    return "common/types/value/rel";
}

test "rel module path" {
    try std.testing.expectEqualStrings("common/types/value/rel", modulePath());
}

test "rel resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/types/value/rel"));
}
