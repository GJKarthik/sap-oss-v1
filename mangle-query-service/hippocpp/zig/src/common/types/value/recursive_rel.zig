//! common/types/value/recursive_rel parity module.
...
pub fn modulePath() []const u8 {
    return "common/types/value/recursive_rel";
}

test "recursive_rel module path" {
    try std.testing.expectEqualStrings("common/types/value/recursive_rel", modulePath());
}

test "recursive_rel resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/types/value/recursive_rel"));
}
