...
def get_engine(model_id: str) -> InferenceEngine:
    ...
    if model_id not in _engines:
        # Map model IDs to actual paths
        model_paths = {
            "qwen3.5-0.6b-int8": "./outputs/Qwen3.5-0.6B_int8",
            "qwen3.5-1.8b-int8": "./outputs/Qwen3.5-1.8B_int8",
            "qwen3.5-4b-int8": "./outputs/Qwen3.5-4B_int8",
            "qwen3.5-9b-int4-awq": "./outputs/Qwen3.5-9B_int4_awq",
        }
        
        path = model_paths.get(model_id)
        if not path:
            # Try loading from HuggingFace if local not found
            path = f"Qwen/{model_id.replace('-int8', '').replace('-int4-awq', '')}"
