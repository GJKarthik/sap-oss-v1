...
class TestAdditiveModelForecastTools(TestML_BaseTestClass):
    ...
    def test_fit_and_save(self):
        ...
        result = json.loads(tool.run({"fit_select_statement": "SELECT * FROM #HANAI_DATA_TBL_RAW",
                                      "key": "TIMESTAMP", "endog": "VALUE",
                                      "name": "HANA_AI_MODEL", "version": 1}))
        self.assertTrue(result['fit_select_statement'] == "SELECT * FROM #HANAI_DATA_TBL_RAW")
        self.assertTrue(result['model_storage_name'] == "HANA_AI_MODEL")
    ...
    def test_load_model_and_predict(self):
        ...
        result = json.loads(tool.run({"predict_select_statement": "SELECT * FROM #HANAI_DATA_TBL_PREDICT_RAW",
                                      "key": "TIMESTAMP", "name": "HANA_AI_MODEL", "version": 1}))
        self.assertTrue('SELECT * FROM #' in result["predicted_results_select_statement"])
