"""
vLLM Infrastructure Agent with ODPS + Regulations Integration

This IS the on-premise vLLM backend - handles all confidential data.
Local only - never routes to external services.
Lowest autonomy (L1) - infrastructure requires human oversight.
"""

import json
import urllib.request
from typing import Any, Dict, List, Optional
from datetime import datetime, timezone


class MangleEngine:
    """Mangle query interface for governance rules."""
    
    def __init__(self, rules_paths: Optional[List[str]] = None):
        self.rules_paths = rules_paths or []
        self.facts: Dict[str, Any] = {}
        self._load_rules()
    
    def _load_rules(self):
        self.facts["agent_config"] = {
            ("vllm-agent", "autonomy_level"): "L1",
            ("vllm-agent", "service_name"): "vllm-inference",
            ("vllm-agent", "mcp_endpoint"): "http://localhost:9180/mcp",
            ("vllm-agent", "default_backend"): "local",
        }
        
        self.facts["agent_can_use"] = {
            "complete", "chat", "health_check", "list_models", "mangle_query"
        }
        
        self.facts["agent_requires_approval"] = {
            "load_model", "unload_model", "change_config", "clear_cache"
        }
        
        self.facts["prompting_policy"] = {
            "vllm-inference-service-v1": {
                "max_tokens": 8192,
                "temperature": 0.7,
                "system_prompt": (
                    "You are a secure on-premise AI assistant running on vLLM. "
                    "All data processed here stays on-premise. "
                    "Never send data to external systems. "
                    "Process confidential and restricted data securely."
                )
            }
        }
    
    def query(self, predicate: str, *args) -> List[Dict]:
        # vLLM IS the local backend - always local, never external
        if predicate == "route_to_local":
            return [{"result": True, "reason": "vLLM IS the local backend"}]
        
        if predicate == "route_to_aicore":
            return []  # Never route to external
        
        if predicate == "route_to_external":
            return []  # Never route to external
        
        if predicate == "requires_human_review":
            action = args[0] if args else ""
            if action in self.facts["agent_requires_approval"]:
                return [{"result": True, "action": action}]
            return []
        
        if predicate == "safety_check_passed":
            tool = args[0] if args else ""
            if tool in self.facts["agent_can_use"]:
                return [{"result": True, "tool": tool}]
            return []
        
        if predicate == "get_prompting_policy":
            product_id = args[0] if args else "vllm-inference-service-v1"
            policy = self.facts["prompting_policy"].get(product_id)
            if policy:
                return [policy]
            return []
        
        if predicate == "autonomy_level":
            return [{"level": "L1"}]
        
        return []


class VLLMAgent:
    """
    vLLM Infrastructure Agent - Local only.
    
    This IS the on-premise vLLM backend.
    Lowest autonomy (L1) - infrastructure requires human oversight.
    """
    
    def __init__(self):
        self.mangle = MangleEngine([
            "mangle/domain/agents.mg",
            "../regulations/mangle/rules.mg"
        ])
        # Local endpoint - this IS vLLM
        self.mcp_endpoint = "http://localhost:9180/mcp"
        self.audit_log: List[Dict] = []
    
    async def invoke(self, prompt: str, context: Optional[Dict] = None) -> Dict[str, Any]:
        context = context or {}
        tool = context.get("tool", "complete")
        timestamp = datetime.now(timezone.utc).isoformat()
        
        # Always local - this IS the vLLM backend
        backend = "local"
        endpoint = self.mcp_endpoint
        routing_reason = "vLLM IS the on-premise backend - local only"
        
        # Check if human review required for infrastructure actions
        if self.mangle.query("requires_human_review", tool):
            self._log_audit("pending_approval", tool, backend, prompt)
            return {
                "status": "pending_approval",
                "message": f"Infrastructure action '{tool}' requires human review",
                "tool": tool,
                "backend": backend,
                "timestamp": timestamp
            }
        
        # Safety check
        if not self.mangle.query("safety_check_passed", tool):
            self._log_audit("blocked", tool, backend, prompt)
            return {
                "status": "blocked",
                "message": f"Safety check failed for tool '{tool}'",
                "tool": tool,
                "timestamp": timestamp
            }
        
        # Get prompting policy
        prompting = self.mangle.query("get_prompting_policy", "vllm-inference-service-v1")
        prompting_policy = prompting[0] if prompting else {}
        
        # Execute locally
        try:
            result = await self._call_mcp(endpoint, tool, {
                "messages": json.dumps([{
                    "role": "system", 
                    "content": prompting_policy.get("system_prompt", "")
                }, {
                    "role": "user", 
                    "content": prompt
                }]),
                "max_tokens": prompting_policy.get("max_tokens", 8192),
                "temperature": prompting_policy.get("temperature", 0.7)
            })
            
            self._log_audit("success", tool, backend, prompt)
            
            return {
                "status": "success",
                "backend": backend,
                "routing_reason": routing_reason,
                "result": result,
                "timestamp": timestamp
            }
            
        except Exception as e:
            self._log_audit("error", tool, backend, prompt, str(e))
            return {
                "status": "error",
                "message": str(e),
                "backend": backend,
                "timestamp": timestamp
            }
    
    async def _call_mcp(self, endpoint: str, tool: str, args: Dict) -> Any:
        request_data = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": "tools/call",
            "params": {"name": tool, "arguments": args}
        }
        
        req = urllib.request.Request(
            endpoint,
            data=json.dumps(request_data).encode(),
            headers={"Content-Type": "application/json"},
            method="POST"
        )
        
        with urllib.request.urlopen(req, timeout=120) as resp:
            return json.loads(resp.read().decode())
    
    def _log_audit(self, status: str, tool: str, backend: str, prompt: str, error: str = None):
        entry = {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "agent": "vllm-agent",
            "status": status,
            "tool": tool,
            "backend": backend,
            "prompt_hash": hash(prompt),
            "prompt_length": len(prompt)
        }
        if error:
            entry["error"] = error
        self.audit_log.append(entry)
    
    def get_audit_log(self) -> List[Dict]:
        return self.audit_log
    
    def check_governance(self, prompt: str) -> Dict[str, Any]:
        prompting = self.mangle.query("get_prompting_policy", "vllm-inference-service-v1")
        autonomy = self.mangle.query("autonomy_level")
        
        return {
            "routing": {
                "backend": "local",
                "reason": "vLLM IS the on-premise backend - LOCAL ONLY"
            },
            "autonomy_level": autonomy[0]["level"] if autonomy else "L1",
            "prompting_policy": prompting[0] if prompting else {},
            "data_product": "vllm-inference-service-v1",
            "requires_human_oversight": True,
            "external_routing_allowed": False,
            "data_retention": "no-storage"
        }


def main():
    import asyncio
    
    agent = VLLMAgent()
    
    print("=" * 60)
    print("vLLM Infrastructure Agent - LOCAL ONLY (L1 Autonomy)")
    print("=" * 60)
    
    # Test 1: Chat completion (local)
    print("\n--- Test 1: Chat Completion ---")
    governance = agent.check_governance("Answer this question")
    print(f"Routing: {governance['routing']['backend']}")
    print(f"Reason: {governance['routing']['reason']}")
    print(f"External allowed: {governance['external_routing_allowed']}")
    
    # Test 2: Text completion (local)
    print("\n--- Test 2: Text Completion ---")
    governance = agent.check_governance("Complete this sentence")
    print(f"Routing: {governance['routing']['backend']}")
    print(f"Autonomy: {governance['autonomy_level']}")
    
    # Test 3: Health check (allowed)
    print("\n--- Test 3: Health Check ---")
    result = asyncio.run(agent.invoke(
        "Check health",
        {"tool": "health_check"}
    ))
    print(f"Status: {result['status']}")
    print(f"Backend: {result.get('backend', 'N/A')}")
    
    # Test 4: Load model (requires approval)
    print("\n--- Test 4: Load Model ---")
    result = asyncio.run(agent.invoke(
        "Load llama-3.1-70b",
        {"tool": "load_model"}
    ))
    print(f"Status: {result['status']}")
    print(f"Message: {result.get('message', '')}")
    
    # Test 5: Change config (requires approval)
    print("\n--- Test 5: Change Config ---")
    result = asyncio.run(agent.invoke(
        "Increase max_tokens to 16384",
        {"tool": "change_config"}
    ))
    print(f"Status: {result['status']}")
    print(f"Message: {result.get('message', '')}")
    
    # Test 6: Audit log
    print("\n--- Audit Log ---")
    for entry in agent.get_audit_log():
        print(f"  [{entry['timestamp']}] {entry['status']} - {entry['tool']} via {entry['backend']}")


if __name__ == "__main__":
    main()