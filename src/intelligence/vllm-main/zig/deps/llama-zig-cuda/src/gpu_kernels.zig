...
pub const GpuKernels = struct {
    ...
    /// Initialize speculative decoding resources.
    pub fn speculativeInit(num_speculative_tokens: u32, hidden_dim: u32, vocab_size: u32) !void {
        if (c.cuda_speculative_init(
            @intCast(num_speculative_tokens),
            @intCast(hidden_dim),
            @intCast(vocab_size),
        ) != 0) {
            return error.SpecInitFailed;
        }
    }

    /// Shutdown speculative decoding, releasing resources.
    pub fn speculativeShutdown() void {
        c.cuda_speculative_shutdown();
    }

    /// Draft K tokens autoregressively using the draft model.
    /// draft_probs layout: [K × vocab_size] full softmax distributions.
    pub fn speculativeDraft(
        draft_tokens: [*]i32,
        draft_probs: [*]f32,
        input: [*]const f32,
        ...
        if (c.cuda_speculative_draft(
            ...
            return error.DraftFailed;
        }
    }

    /// Verify draft tokens against the main model using rejection sampling.
    /// Returns the number of accepted tokens (including possible correction token).
    pub fn speculativeVerify(
        accepted_tokens: [*]i32,
        num_accepted: *i32,
