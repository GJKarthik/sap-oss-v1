...
pub const Model = struct {
    ...
    pub fn forward(self: *Self, token: u32, pos: usize, kv_cache: *KVCache) []f32 {
        ...
        for (self.weights.layers, 0..) |layer, layer_idx| {
            ...
            // QKV projections
            if (layer.wqkv) |wqkv| {
                // Fused QKV (phi2): weight layout is [(q_size + 2*kv_size), dim] row-major
                // Slice the concatenated weight matrix into Q, K, V portions
                const q_size = n_heads * head_dim;
                const kv_size = n_kv_heads * head_dim;
                const wq_slice = wqkv[0 .. q_size * dim];
                const wk_slice = wqkv[q_size * dim .. (q_size + kv_size) * dim];
                const wv_slice = wqkv[(q_size + kv_size) * dim .. (q_size + 2 * kv_size) * dim];
                kernels.matvec(self.q, wq_slice, self.xb, q_size, dim);
                kernels.matvec(self.k, wk_slice, self.xb, kv_size, dim);
                kernels.matvec(self.v, wv_slice, self.xb, kv_size, dim);
            } else {
                // Separate Q, K, V
