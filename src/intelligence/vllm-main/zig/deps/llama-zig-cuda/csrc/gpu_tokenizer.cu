...
__global__ void bytes_to_tokens_kernel(
    ...
    // Clamp to MAX_SEQ_LEN to prevent out-of-bounds writes
    if (input_len > MAX_SEQ_LEN) input_len = MAX_SEQ_LEN;
...
__global__ void bpe_merge_kernel(
    ...
    // Guard against oversized sequences
    if (num_tokens <= 1 || num_tokens > MAX_SEQ_LEN) return;
...
/**
 * Alternative tokenization using hash-based longest-match (greedy).
 *
 * This mode does NOT apply BPE merges. Instead, at each position in the input
 * it finds the longest vocabulary token that matches and greedily consumes it.
 * This is faster than iterative BPE for vocabularies where longest-match
 * produces acceptable results (e.g., SentencePiece unigram models).
 *
 * Requires vocab_hashes to be precomputed and uploaded. Use gpu_tokenize_batch()
 * with num_bpe_merges for standard BPE tokenization instead.
 *
 * @param output_tokens   Output: [batch, MAX_SEQ_LEN]
 * @param output_lengths  Output: [batch]
 * @param input_texts     Input: [batch, max_input_len] raw text
 * @param input_lengths   Input: [batch] length of each text
 * @param vocab_hashes    Precomputed FNV-1a hashes per vocab token [vocab_size]
 * @param batch_size      Number of input texts
 * @param max_input_len   Maximum input text length
 */
extern "C" int gpu_tokenize_batch_longest_match(
    int* output_tokens,
    int* output_lengths,
    const char* input_texts,
    const int* input_lengths,
    const uint64_t* vocab_hashes,
    int batch_size,
    int max_input_len
) {
    if (!g_vocab.initialized) return -1;

    // Device allocations
    char* d_input = nullptr;
    int* d_input_lens = nullptr;
    int* d_match_tokens = nullptr;
    int* d_match_lengths = nullptr;
    int* d_out_tokens = nullptr;
    int* d_out_lengths = nullptr;
    uint64_t* d_vocab_hashes = nullptr;

    size_t match_size = (size_t)batch_size * max_input_len * sizeof(int);

    if (cudaMalloc(&d_input, (size_t)batch_size * max_input_len) != cudaSuccess) return -1;
    if (cudaMalloc(&d_input_lens, batch_size * sizeof(int)) != cudaSuccess) goto lm_fail;
    if (cudaMalloc(&d_match_tokens, match_size) != cudaSuccess) goto lm_fail;
    if (cudaMalloc(&d_match_lengths, match_size) != cudaSuccess) goto lm_fail;
    if (cudaMalloc(&d_out_tokens, batch_size * MAX_SEQ_LEN * sizeof(int)) != cudaSuccess) goto lm_fail;
    if (cudaMalloc(&d_out_lengths, batch_size * sizeof(int)) != cudaSuccess) goto lm_fail;
    if (cudaMalloc(&d_vocab_hashes, g_vocab.vocab_size * sizeof(uint64_t)) != cudaSuccess) goto lm_fail;

    cudaMemcpy(d_input, input_texts, (size_t)batch_size * max_input_len, cudaMemcpyHostToDevice);
    cudaMemcpy(d_input_lens, input_lengths, batch_size * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_vocab_hashes, vocab_hashes, g_vocab.vocab_size * sizeof(uint64_t), cudaMemcpyHostToDevice);

    cudaMemset(d_match_tokens, 0xFF, match_size);  // -1
    cudaMemset(d_match_lengths, 0, match_size);

    {
        // Step 1: Per-position longest match
        int max_len = 0;
        for (int i = 0; i < batch_size; i++) {
            if (input_lengths[i] > max_len) max_len = input_lengths[i];
        }
        int blocks_y = (max_len + BLOCK_SIZE - 1) / BLOCK_SIZE;
        dim3 grid(batch_size, blocks_y);

        hash_tokenize_kernel<<<grid, BLOCK_SIZE>>>(
            d_match_tokens, d_match_lengths,
            d_input, d_input_lens,
            g_vocab.tokens, g_vocab.token_lengths,
            d_vocab_hashes,
            batch_size, max_input_len, g_vocab.vocab_size
        );
    }

    {
        ...
        greedy_select_kernel<<<blocks, BLOCK_SIZE>>>(
            d_out_tokens, d_out_lengths,
            d_match_tokens, d_match_lengths,
            d_input_lens,
            batch_size, max_input_len
        );
    ...
    cudaMemcpy(output_tokens, d_out_tokens, batch_size * MAX_SEQ_LEN * sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(output_lengths, d_out_lengths, batch_size * sizeof(int), cudaMemcpyDeviceToHost);

    cudaFree(d_input); cudaFree(d_input_lens);
    cudaFree(d_match_tokens); cudaFree(d_match_lengths);
    cudaFree(d_out_tokens); cudaFree(d_out_lengths);
    cudaFree(d_vocab_hashes);
    return (cudaGetLastError() == cudaSuccess) ? 0 : -1;

lm_fail:
    if (d_input) cudaFree(d_input);
    if (d_input_lens) cudaFree(d_input_lens);
    if (d_match_tokens) cudaFree(d_match_tokens);
    if (d_match_lengths) cudaFree(d_match_lengths);
    if (d_out_tokens) cudaFree(d_out_tokens);
    if (d_out_lengths) cudaFree(d_out_lengths);
    if (d_vocab_hashes) cudaFree(d_vocab_hashes);
    return -1;
}
