...
- Be concise in your reasoning steps"""
    ...
    fn get_reasoning_steps(self, result: Dict[String, String]) -> List[String]:
        ...
        Returns:
            List of reasoning steps. Returns empty list if:
            - "reasoning" field is missing from result
            - "reasoning" field is empty
            - Result contains an error
        """
        var steps = List[String]()
        
        # Guard: Check for error result
        if "error" in result:
            return steps
        
        # Guard: Check if reasoning field exists
        if "reasoning" not in result:
            return steps
        
        var reasoning = result["reasoning"]
        
        # Guard: Check if reasoning value is non-empty
        if len(reasoning) == 0:
            return steps
        
        var parts = reasoning.split("|")
        for i in range(len(parts)):
            var step = parts[i].strip()
    ...
    fn has_reasoning(self, result: Dict[String, String]) -> Bool:
        """Check if result contains valid reasoning.
        
        Returns:
            True if result has non-empty reasoning field, False otherwise.
        """
        if "error" in result:
            return False
        if "reasoning" not in result:
