"""
Unified SAP Elasticsearch + Mangle Query Service Entry Point.

Serves three ports from a single process:
  PORT        (default 8080) — Mangle query API (HANA vector, OpenAI-compatible)
  ES_OAI_PORT (default 9201) — ES OpenAI-compatible server (sap_openai_server)
  MCP_PORT    (default 9120) — Elasticsearch MCP server (started separately via entrypoint)

The Go gRPC Mangle engine (mangle-engine binary) is started as a sidecar
subprocess on GRPC_PORT (default 50051) if the binary is present on PATH.
"""
...
class ApplicationState:
    ...
    def __init__(self):
        ...
        self._metrics = None
        # Go gRPC sidecar
        self._grpc_proc: subprocess.Popen | None = None
    
    def _start_grpc_sidecar(self) -> None:
        """Start the Go Mangle gRPC engine as a background subprocess.

        The binary is expected at ``MANGLE_ENGINE_BIN`` (default: ``mangle-engine``
        on PATH).  If the binary is not found the sidecar is silently skipped —
        the Python MangleEngine offline replica takes over in _governance_check.
        """
        import shutil
        bin_path = os.getenv("MANGLE_ENGINE_BIN", "mangle-engine")
        if not shutil.which(bin_path):
            logger.warning(
                "Go Mangle engine binary '%s' not found; gRPC governance unavailable. "
                "Python offline replica will be used as fallback.",
                bin_path,
            )
            return
        rules_dir = os.getenv("MQS_RULES_DIR", os.path.join(os.path.dirname(__file__), "..", "..", "rules"))
        grpc_port = os.getenv("GRPC_PORT", "50051")
        try:
            self._grpc_proc = subprocess.Popen(
                [bin_path],
                env={**os.environ, "MQS_RULES_DIR": rules_dir, "GRPC_PORT": grpc_port},
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
            )
            logger.info("Go Mangle engine started (pid=%d, port=%s)", self._grpc_proc.pid, grpc_port)
        except Exception as exc:
            logger.warning("Failed to start Go Mangle engine: %s", exc)
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # 0. Go gRPC sidecar (non-blocking; failures are soft)
            self._start_grpc_sidecar()
    ...
    async def shutdown(self) -> None:
        ...
        try:
            # Terminate Go gRPC sidecar
            if self._grpc_proc is not None and self._grpc_proc.poll() is None:
                self._grpc_proc.terminate()
                try:
                    self._grpc_proc.wait(timeout=5)
                except subprocess.TimeoutExpired:
                    self._grpc_proc.kill()
                logger.info("Go Mangle engine stopped")
...
# ============================================================================
# ES OpenAI-compatible sub-application (port 9201)
# Mounted at /es-oai for combined deployments; also served standalone on
# ES_OAI_PORT when USE_ES_OAI_SERVER=true.
# ============================================================================

try:
    from sap_openai_server.server import app as _es_oai_app  # type: ignore
    app.mount("/es-oai", _es_oai_app)
    logger.info("ES OpenAI-compatible server mounted at /es-oai")
except Exception as _mount_err:
    logger.warning("Could not mount ES OpenAI server: %s", _mount_err)
...
def _run_es_oai_server(host: str) -> None:
    """Start the ES sap_openai_server on ES_OAI_PORT in a daemon thread."""
    es_oai_port = int(os.getenv("ES_OAI_PORT", "9201"))
    try:
        uvicorn.run(
            "sap_openai_server.server:app",
            host=host,
            port=es_oai_port,
        ...
        logger.warning("ES OpenAI server failed to start: %s", exc)
...
def main():
    ...
    # Start ES OpenAI server on port 9201 in a background daemon thread
    if os.getenv("USE_ES_OAI_SERVER", "true").lower() == "true":
        es_oai_thread = threading.Thread(target=_run_es_oai_server, args=(host,), daemon=True)
        es_oai_thread.start()
        logger.info("ES OpenAI-compatible server starting on port %s", os.getenv("ES_OAI_PORT", "9201"))
