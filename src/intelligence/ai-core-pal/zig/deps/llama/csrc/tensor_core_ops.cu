...
__global__ void fused_scale_softmax_fp16_kernel(
    ...
    // Load and scale
    float val = (idx < n) ? __half2float(data[idx]) * scale : -INFINITY;
    sdata[tid] = val;
    __syncthreads();
    ...
    // Compute exp and sum
    val = (idx < n) ? expf((__half2float(data[idx]) * scale) - max_val) : 0.0f;
    sdata[tid] = val;
    __syncthreads();
    ...
    if (idx < n) {
        data[idx] = __float2half(
            expf((__half2float(data[idx]) * scale) - max_val) / sum
        );
    }
}
...
__global__ void rms_norm_fp16_kernel(
    ...
    float val = (idx < n) ? __half2float(src[idx]) : 0.0f;
    sdata[tid] = val * val;
    __syncthreads();
    ...
    if (idx < n) {
        ...
        dst[idx] = __float2half(__half2float(src[idx]) * rms * w);
    }
}
