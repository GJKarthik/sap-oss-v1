...
pub const MetalBackend = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: MetalConfig) !*MetalBackend {
        ...
        if (comptime builtin.os.tag == .macos and !builtin.is_test) {
            ...
            device = @import("metal_bindings.zig").getDevice();
            if (device != null) {
                device_name = detectDeviceName(allocator) catch "Apple GPU";
            }
            _ = &command_queue; // Will be set by real Metal init
        }
    ...
    pub fn embeddings(
        ...
        // GPU path (only in production builds)
    ...
    pub fn matmul(
        ...
        // Metal MPS matmul would go here
        const result = try self.matmulCpuFallback(a, b, c, m, n, k);
    ...
    pub fn softmax(
        ...
        // Optimized softmax computation (CPU, GPU in production)
        for (0..batch_size) |b| {
