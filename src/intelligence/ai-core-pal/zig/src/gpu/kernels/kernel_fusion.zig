//! GPU Kernel Fusion Engine for MCP-PAL
...
//!   Stage 3: Inference kernel generates embeddings (no CPU roundtrip)
...
pub const KernelFusionPipeline = struct {
        ...
        if (self.config.enable_json_parser) {
            ...
            if (self.gpu_dispatch) |dispatch| {
                if (dispatch.parse_fn) |parse_fn| {
                    if (parse_fn(dispatch.ctx, raw_json, &parse_result)) {
                        parse_used_gpu = true;
            ...
            if (!parse_used_gpu) {
                parse_result = self.json_parser_kernel.parse(raw_json);
            }
        ...
        if (self.config.enable_tokenizer) {
            ...
            if (self.gpu_dispatch) |dispatch| {
                if (dispatch.tokenize_fn) |tokenize_fn| {
                    if (self.config.enable_json_parser and result.text_end > result.text_start and result.text_end <= raw_json.len) {
                        const text_slice = raw_json[result.text_start..result.text_end];
            ...
            if (!tokenize_used_gpu) {
                token_result = if (self.config.enable_json_parser)
    ...
    /// Generate embeddings (GPU when available, CPU fallback)
    fn simulateInference(self: *KernelFusionPipeline, num_tokens: u32) !void {
        const embedding_dim = self.config.embedding_dim;
        
        // Try GPU embed dispatch first
        if (self.gpu_dispatch) |dispatch| {
            if (dispatch.embed_fn) |embed_fn| {
                if (embed_fn(dispatch.ctx, self.token_buffer[0..num_tokens], self.output_buffer, embedding_dim)) {
                    _ = self.gpu_dispatches.fetchAdd(1, .monotonic);
                    return;
                }
            }
        ...
        // CPU fallback: deterministic embedding
        for (0..num_tokens) |t| {
