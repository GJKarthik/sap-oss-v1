...
const parser = @import("mangle_parser.zig");
const loader = @import("mangle_loader.zig");
...
pub const MangleQueryEngine = struct {
    ...
    pub fn init(allocator: Allocator, mangle_dir: []const u8) MangleQueryEngine {
        return .{
            ...
            .program = null,
            .injected_facts = std.ArrayList(parser.Fact).init(allocator),
        };
    }
    ...
    pub fn queryFacts(self: *MangleQueryEngine, name: []const u8) []const parser.Fact {
        var matches = std.ArrayList(parser.Fact).init(self.allocator);
