...
# ============================================================================
# Model Registry — derived from the single source of truth
# ============================================================================

def _build_available_models() -> List[ModelObject]:
...
# Lazy-initialised so the import cycle stays clean
_available_models: Optional[List[ModelObject]] = None


def _get_available_models() -> List[ModelObject]:
...
async def list_models(authorization: Optional[str] = Header(None)):
    ...
    return ModelList(data=_get_available_models())
...
async def create_chat_completion(
    ...
    known_ids = [m.id for m in _get_available_models()]
    if request.model not in known_ids:
        raise HTTPException(
            status_code=404,
...
def _generate_completion(request: ChatCompletionRequest) -> str:
    ...
    Attempts real inference via InferenceEngine. Falls back to a
    deterministic placeholder when the model cannot be loaded (e.g. no
    GPU, missing weights) so the API always returns a valid response.
    """
    # Build a chat-style prompt from the messages
    ...
    try:
        ...
        # Fallback — keeps the API functional without a GPU / model weights
        last_message = ""
        for m in reversed(request.messages):
            if m.content and m.role == "user":
                ...
                break
        return (
            f"This is a response from {request.model}. "
            f"In production, this would be actual model inference. "
            f"Your message was: {last_message[:100]}..."
        )
