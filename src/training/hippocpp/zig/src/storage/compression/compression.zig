    ...
    pub fn encode(self: *RLEEncoder, values: []const i64) !CompressionBuffer {
        ...
        for (values[1..]) |value| {
            if (value == current_value) {
                ...
                try buffer.data.appendSlice(self.allocator, std.mem.asBytes(&current_value));
                try buffer.data.appendSlice(self.allocator, std.mem.asBytes(&run_length));
                current_value = value;
                run_length = 1;
            }
        }
...
pub const DictionaryEncoder = struct {
    ...
    pub fn deinit(self: *DictionaryEncoder) void {
        ...
        self.reverse_dict.deinit(self.allocator);
    }
    ...
    pub fn encode(self: *DictionaryEncoder, values: []const []const u8) !CompressionBuffer {
        ...
        for (values) |value| {
            ...
            if (self.dictionary.get(value)) |idx| {
                ...
                try self.reverse_dict.append(self.allocator, value);
                try indices.append(self.allocator, new_idx);
            }
        }
        
        // Write dictionary size
        const dict_size: u32 = @intCast(self.reverse_dict.items.len);
        try buffer.data.appendSlice(self.allocator, std.mem.asBytes(&dict_size));
        
        // Write dictionary entries
        for (self.reverse_dict.items) |entry| {
            const len: u32 = @intCast(entry.len);
            try buffer.data.appendSlice(self.allocator, std.mem.asBytes(&len));
            try buffer.data.appendSlice(self.allocator, entry);
        }
        
        // Write indices
        for (indices.items) |idx| {
            try buffer.data.appendSlice(self.allocator, std.mem.asBytes(&idx));
        }
...
pub const BitPackingEncoder = struct {
    ...
    pub fn encode(self: *BitPackingEncoder, values: []const u64) !CompressionBuffer {
        ...
        try buffer.data.append(self.allocator, bit_width);
        const num_vals: u32 = @intCast(values.len);
        try buffer.data.appendSlice(self.allocator, std.mem.asBytes(&num_vals));
        ...
        for (values) |value| {
            ...
            bit_buffer |= (value & mask) << @as(u6, @intCast(bits_in_buffer));
            bits_in_buffer += bit_width;
            
            while (bits_in_buffer >= 8) {
                try buffer.data.append(self.allocator, @truncate(bit_buffer & 0xFF));
                bit_buffer >>= 8;
                bits_in_buffer -= 8;
            }
        }
    ...
    pub fn decode(self: *BitPackingEncoder, compressed: []const u8) !std.ArrayList(u64) {
        ...
        while (decoded < num_values) {
            ...
            while (bits_in_buffer < bit_width and data_pos < compressed.len) {
                bit_buffer |= @as(u64, compressed[data_pos]) << @as(u6, @intCast(bits_in_buffer));
                bits_in_buffer += 8;
                data_pos += 1;
            }
            ...
            // Extract value
            const mask = (@as(u64, 1) << @as(u6, @intCast(bit_width))) - 1;
            try result.append(self.allocator, bit_buffer & mask);
            bit_buffer >>= @as(u6, @intCast(bit_width));
            bits_in_buffer -= bit_width;
            decoded += 1;
        }
...
pub const DeltaEncoder = struct {
    ...
    pub fn encode(self: *DeltaEncoder, values: []const i64) !CompressionBuffer {
        ...
        // Write first value
        try buffer.data.appendSlice(self.allocator, std.mem.asBytes(&values[0]));
    ...
    fn writeVarint(self: *DeltaEncoder, data: *std.ArrayList(u8), value: i64) !void {
        ...
        var v = zigzag;
        while (v >= 0x80) {
            try data.append(self.allocator, @truncate((v & 0x7F)));
            v >>= 7;
        }
        try data.append(self.allocator, @truncate(v));
    }
    ...
    pub fn decode(self: *DeltaEncoder, compressed: []const u8, num_values: usize) !std.ArrayList(i64) {
        ...
        try result.append(self.allocator, prev);
...
pub const ConstantEncoder = struct {
    ...
    pub fn encode(self: *ConstantEncoder, values: []const i64) !CompressionBuffer {
        ...
        try buffer.data.appendSlice(self.allocator, std.mem.asBytes(&count));
