//! storage/stats/column_stats parity module.
...
pub fn modulePath() []const u8 {
    return "storage/stats/column_stats";
}

test "column_stats module path" {
    try std.testing.expectEqualStrings("storage/stats/column_stats", modulePath());
}

test "column_stats resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/stats/column_stats"));
}
