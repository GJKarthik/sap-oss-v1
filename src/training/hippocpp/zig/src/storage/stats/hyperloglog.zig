//! storage/stats/hyperloglog parity module.
...
pub fn modulePath() []const u8 {
    return "storage/stats/hyperloglog";
}

test "hyperloglog module path" {
    try std.testing.expectEqualStrings("storage/stats/hyperloglog", modulePath());
}

test "hyperloglog resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/stats/hyperloglog"));
}
