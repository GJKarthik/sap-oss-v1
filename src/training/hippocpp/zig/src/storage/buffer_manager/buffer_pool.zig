...
pub const BufferPool = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, page_size: usize, num_frames: usize) !BufferPool {
        ...
        // Create frames
        const frames = try allocator.alloc(BufferFrame, num_frames);
        ...
        var free_frames: std.ArrayList(u32) = .{};
        var i: u32 = 0;
        while (i < num_frames) : (i += 1) {
            try free_frames.append(allocator, i);
        }
        
        return .{
            .allocator = allocator,
            .page_size = page_size,
            .num_frames = @intCast(num_frames),
            .frames = frames,
            .page_data = page_data,
    ...
    fn getVictimFrame(self: *BufferPool) !u32 {
        ...
        if (self.free_frames.items.len > 0) {
            return self.free_frames.pop() orelse return error.NoFreeFrames;
        }
