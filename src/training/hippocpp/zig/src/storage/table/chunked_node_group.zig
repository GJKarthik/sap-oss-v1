//! storage/table/chunked_node_group parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/chunked_node_group";
}

test "chunked_node_group module path" {
    try std.testing.expectEqualStrings("storage/table/chunked_node_group", modulePath());
}

test "chunked_node_group resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/chunked_node_group"));
}
