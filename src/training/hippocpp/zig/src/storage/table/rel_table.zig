...
pub const CSRList = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) CSRList {
        ...
        try csr.offsets.append(allocator, 0);
        return csr;
    }
    ...
    pub fn deinit(self: *CSRList) void {
        ...
        self.rel_ids.deinit(self.allocator);
    }
...
pub const RelTableSchema = struct {
    ...
    pub fn init(_: std.mem.Allocator, table_id: u64, name: []const u8, src_id: u64, dst_id: u64) RelTableSchema {
        return .{
            .table_id = table_id,
            .name = name,
            .src_table_id = src_id,
...
pub const RelTable = struct {
    ...
    pub fn deinit(self: *RelTable) void {
        ...
        self.property_columns.deinit(self.allocator);
    ...
    pub fn insertRelationship(self: *RelTable, src_node: u64, dst_node: u64) !u64 {
        ...
        try self.relationships.append(self.allocator, RelEntry.init(src_node, dst_node, rel_id));
    ...
    pub fn buildCSR(self: *RelTable, num_src_nodes: usize, num_dst_nodes: usize) !void {
        ...
        try self.fwd_csr.offsets.append(self.allocator, 0);
        
        for (self.relationships.items) |rel| {
            // Fill gaps for nodes with no outgoing edges
            while (current_src < rel.src_node) {
                try self.fwd_csr.offsets.append(self.allocator, @intCast(self.fwd_csr.neighbors.items.len));
                current_src += 1;
            }
            
            try self.fwd_csr.neighbors.append(self.allocator, rel.dst_node);
            try self.fwd_csr.rel_ids.append(self.allocator, rel.rel_id);
        }
        ...
        const bwd_edges = try self.allocator.alloc(RelEntry, self.relationships.items.len);
        ...
        try self.bwd_csr.offsets.append(self.allocator, 0);
        
        for (bwd_edges) |rel| {
            while (current_dst < rel.dst_node) {
                try self.bwd_csr.offsets.append(self.allocator, @intCast(self.bwd_csr.neighbors.items.len));
                current_dst += 1;
            }
            
            try self.bwd_csr.neighbors.append(self.allocator, rel.src_node);
            try self.bwd_csr.rel_ids.append(self.allocator, rel.rel_id);
        }
...
test "rel table basic" {
    ...
    const schema = RelTableSchema.init(allocator, 0, "KNOWS", 0, 0);
...
test "rel table csr" {
    ...
    const schema = RelTableSchema.init(allocator, 0, "FOLLOWS", 0, 0);
...
test "rel table iterator" {
    ...
    const schema = RelTableSchema.init(allocator, 0, "EDGE", 0, 0);
