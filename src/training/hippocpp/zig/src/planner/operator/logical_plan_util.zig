//! planner/operator/logical_plan_util parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/operator/logical_plan_util";
}

test "logical_plan_util planner path" {
    try std.testing.expectEqualStrings("planner/operator/logical_plan_util", plannerPath());
}

test "logical_plan_util planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/operator/logical_plan_util"));
}
