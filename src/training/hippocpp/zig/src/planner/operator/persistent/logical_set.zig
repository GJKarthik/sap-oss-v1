//! planner/operator/persistent/logical_set parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/operator/persistent/logical_set";
}

test "logical_set planner path" {
    try std.testing.expectEqualStrings("planner/operator/persistent/logical_set", plannerPath());
}

test "logical_set planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/operator/persistent/logical_set"));
}
