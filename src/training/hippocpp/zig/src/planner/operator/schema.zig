//! planner/operator/schema parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/operator/schema";
}

test "schema planner path" {
    try std.testing.expectEqualStrings("planner/operator/schema", plannerPath());
}

test "schema planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/operator/schema"));
}
