//! planner/join_order/join_tree parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/join_order/join_tree";
}

test "join_tree planner path" {
    try std.testing.expectEqualStrings("planner/join_order/join_tree", plannerPath());
}

test "join_tree planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/join_order/join_tree"));
}
