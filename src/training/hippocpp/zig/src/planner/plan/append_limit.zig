//! planner/plan/append_limit parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_limit";
}

test "append_limit planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_limit", plannerPath());
}

test "append_limit planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_limit"));
}
