//! planner/plan/append_unwind parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_unwind";
}

test "append_unwind planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_unwind", plannerPath());
}

test "append_unwind planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_unwind"));
}
