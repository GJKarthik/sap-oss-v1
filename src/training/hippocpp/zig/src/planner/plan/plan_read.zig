//! planner/plan/plan_read parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/plan_read";
}

test "plan_read planner path" {
    try std.testing.expectEqualStrings("planner/plan/plan_read", plannerPath());
}

test "plan_read planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/plan_read"));
}
