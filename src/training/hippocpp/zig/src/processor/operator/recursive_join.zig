...
const physical = @import("physical_operator");
const gds = @import("gds");
const rec_joins = @import("rec_joins");
const var_path = @import("var_path");
...
pub const RecursiveJoinOperator = struct {
    ...
    pub fn deinit(self: *RecursiveJoinOperator) void {
        self.result_pairs.deinit(self.allocator);
    ...
    pub fn execute(self: *RecursiveJoinOperator) !void {
        ...
        for (result.pairs.items) |pair| {
            try self.result_pairs.append(self.allocator, pair);
        }
...
pub const PathScanOperator = struct {
    ...
    pub fn scan(
        ...
        for (result.paths.items) |*path| {
            ...
            try self.paths.append(self.allocator, cloned);
        }
