...
const Engine = struct {
    ...
    fn deinit(self: *Engine) void {
        ...
        self.node_tables.deinit(self.allocator);
        ...
        self.rel_tables.deinit(self.allocator);
    }
    ...
    fn execute(self: *Engine, query_in: []const u8, params: ?*const std.json.ObjectMap, result: *ResultSet) !void {
        ...
        if (startsWithAsciiNoCase(query, "MATCH (")) {
            if (indexOfAsciiNoCase(query, " CREATE ") != null and std.mem.indexOf(u8, query, ")-[") != null and std.mem.indexOf(u8, query, "]->(") != null) {
                try self.executeMatchCreateRelationship(query, params, result);
                return;
            }
    ...
    fn formatCountOutputName(self: *Engine, count_expr: []const u8, distinct: bool) ![]u8 {
        ...
        if (distinct) {
            return try std.fmt.allocPrint(self.allocator, "COUNT(DISTINCT {s})", .{count_expr});
        }
        return try std.fmt.allocPrint(self.allocator, "COUNT({s})", .{count_expr});
    }
    ...
    fn createNodeFromPattern(
        ...
        while (props.next()) |raw_prop| {
            ...
            const literal = try self.parseLiteralOrParameter(value_text, params);
    ...
    fn executeCreateNode(
        ...
        const created = try self.createNodeFromPattern(create_part, params);
    ...
    fn executeCreateRelationship(
        ...
        const left_ref = try self.createNodeFromPattern(left_pattern, params);
        const right_ref = try self.createNodeFromPattern(right_pattern, params);
        ...
        if (return_projection) |projection_part| {
            ...
            if (!has_count_projection) {
                ...
                for (projection_terms.items) |raw_term| {
                    ...
                    if (self.parsePropertyExprOptional(projection_term.expr, parsed.left_var)) |col_name| {
                        ...
                        try result.columns.append(self.allocator, try self.allocator.dupe(u8, projection_term.alias orelse projection_term.expr));
                        try result.types.append(self.allocator, typeName(ty));
                        try return_sources.append(self.allocator, .{ .left_col = col_idx });
                        continue;
                    }

                    if (self.parsePropertyExprOptional(projection_term.expr, parsed.right_var)) |col_name| {
                    ...
                    if (self.parsePropertyExprOptional(projection_term.expr, rel_scope_var)) |col_name| {
                    ...
                    if (Engine.parsePropertyAccessExpr(projection_term.expr)) |property_expr| {
                    ...
                    if (Engine.scopeVariableForUnknownExpr(projection_term.expr)) |unknown_var| {
                        try self.failUserFmt("Binder exception: Variable {s} is not in scope.", .{unknown_var});
                        unreachable;
                    }

                    var scalar = try self.evaluateReturnScalarExpr(projection_term.expr, params);
                    defer scalar.cell.deinit(self.allocator);
                    try result.columns.append(
                        self.allocator,
                        try self.allocator.dupe(u8, projection_term.alias orelse scalar.default_alias),
                    );
                    try result.types.append(self.allocator, scalar.type_name);
                    try return_sources.append(self.allocator, .{ .scalar_expr = projection_term.expr });
                }
            }
        }
    ...
    fn parseMatchNodePattern(pattern_in: []const u8) !struct { var_name: []const u8, table_name: []const u8 } {
        ...
        return .{
            ...
            .table_name = std.mem.trim(u8, inner[colon_idx + 1 ..], " \t\n\r"),
        };
    }
    ...
    fn parseNodeSetAssignments(
        ...
        while (it.next()) |raw| {
            ...
            try out_assignments.append(self.allocator, try self.parseNodeSetAssignment(table, var_name, clause, params));
        }
    }
    ...
    fn executeMatchSet(
        ...
        try self.parseNodeSetAssignments(table, head.var_name, set_text, params, &assignments);
        ...
        if (try parseCountProjectionClause(projection_part)) |count_clause| {
            ...
            if (count_clause.distinct) {
                ...
                for (indices.items) |rel_idx| {
                    ...
                    var seen_values = .{};
                    errdefer seen_values.deinit(self.allocator);
...
pub fn main() !void {
    ...
    defer _ = gpa_impl.deinit(self.allocator);
    const allocator = gpa_impl.allocator();

    var engine = Engine.init(allocator);
    defer engine.deinit(self.allocator);
