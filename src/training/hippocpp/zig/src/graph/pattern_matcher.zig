...
pub const EdgePattern = struct {
    ...
    pub fn addType(self: *Self, rel_type: []const u8) !void {
        try self.types.append(self.allocator, rel_type);
    }
...
pub const PathPattern = struct {
    ...
    pub fn addNode(self: *Self, node: NodePattern) !void {
        try self.elements.append(self.allocator, .{ .NODE = node });
    }
    
    pub fn addEdge(self: *Self, edge: EdgePattern) !void {
        try self.elements.append(self.allocator, .{ .EDGE = edge });
    }
...
test "path pattern" {
    ...
    defer path_pattern.deinit(std.testing.allocator);
