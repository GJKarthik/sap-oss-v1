//! graph/graph_entry_set parity module.
...
pub fn modulePath() []const u8 {
    return "graph/graph_entry_set";
}

test "graph_entry_set module path" {
    try std.testing.expectEqualStrings("graph/graph_entry_set", modulePath());
}

test "graph_entry_set resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("graph/graph_entry_set"));
}
