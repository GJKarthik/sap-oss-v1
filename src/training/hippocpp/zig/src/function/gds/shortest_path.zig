...
test "dijkstra simple graph" {
    ...
    var n1 = .{};
    try n1.append(std.testing.allocator, .{ .target = 2, .weight = 1.0 });
    try n1.append(std.testing.allocator, .{ .target = 3, .weight = 4.0 });
    try adj.put(1, n1);
    
    var n2 = .{};
    try n2.append(std.testing.allocator, .{ .target = 3, .weight = 2.0 });
    try adj.put(2, n2);
...
test "dijkstra with target" {
    ...
    var n2 = .{};
    try n2.append(std.testing.allocator, .{ .target = 3, .weight = 1.0 });
    try adj.put(2, n2);
