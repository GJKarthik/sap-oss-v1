...
pub fn compare(comptime T: type, left: T, right: T, op: ComparisonType) bool {
    return switch (op) {
        ...
        .LESS_THAN => if (@typeInfo(T) == .bool) false else left < right,
        .LESS_THAN_OR_EQUAL => if (@typeInfo(T) == .bool) (left == right) else left <= right,
        .GREATER_THAN => if (@typeInfo(T) == .bool) false else left > right,
        .GREATER_THAN_OR_EQUAL => if (@typeInfo(T) == .bool) (left == right) else left >= right,
    };
}
...
pub fn registerFunctions(catalog: *FunctionCatalog) !void {
    ...
    for (numeric_types) |input_type| {
        // EQUALS
        const eq = try createEqualsFunction(catalog.allocator, input_type);
        try catalog.registerScalar(eq);
        
        // LESS_THAN
        const lt = try createLessThanFunction(catalog.allocator, input_type);
        try catalog.registerScalar(lt);
        
        // GREATER_THAN  
        const gt = try createGreaterThanFunction(catalog.allocator, input_type);
