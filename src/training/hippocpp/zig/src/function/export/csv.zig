...
pub const CSVWriter = struct {
    ...
    pub fn writeHeader(self: *CSVWriter, columns: []const []const u8) !void {
        ...
        for (columns, 0..) |col, i| {
            if (i > 0) {
                try self.output.append(self.allocator, self.config.delimiter);
            }
            try self.writeQuotedString(col);
        }
        try self.output.appendSlice(self.allocator, self.config.newline);
    }
    ...
    pub fn writeValue(self: *CSVWriter, value: CSVValue) !void {
        ...
        switch (value) {
            .null_value => try self.output.appendSlice(self.allocator, self.config.null_string),
    ...
    fn writeQuotedString(self: *CSVWriter, s: []const u8) !void {
        ...
        if (needs_quote) {
            try self.output.append(self.allocator, self.config.quote);
            for (s) |c| {
                if (c == self.config.quote) {
                    try self.output.append(self.allocator, self.config.escape);
                }
        ...
        while (!self.isAtEnd()) {
            ...
            if (c == self.config.quote) {
                if (!self.isAtEnd() and self.peek() == self.config.quote) {
                    // Escaped quote
                    self.advance();
                    try result.append(self.allocator, self.config.quote);
                } else {
                    // End of quoted field
                    break;
                }
...
test "csv reader basic" {
    ...
    defer row1.deinit(std.testing.allocator);
