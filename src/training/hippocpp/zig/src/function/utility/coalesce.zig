//! function/utility/coalesce parity module.
...
pub fn modulePath() []const u8 {
    return "function/utility/coalesce";
}

test "coalesce module path" {
    try std.testing.expectEqualStrings("function/utility/coalesce", modulePath());
}

test "coalesce canonical name fallback" {
    var module = Module.init("");
    try std.testing.expect(module.matches("function/utility/coalesce"));
}
