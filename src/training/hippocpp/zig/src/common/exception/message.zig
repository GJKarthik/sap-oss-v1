//! common/exception/message parity module.
...
pub fn modulePath() []const u8 {
    return "common/exception/message";
}

test "message module path" {
    try std.testing.expectEqualStrings("common/exception/message", modulePath());
}

test "message resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/exception/message"));
}
