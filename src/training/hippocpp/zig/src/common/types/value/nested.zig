//! common/types/value/nested parity module.
...
pub fn modulePath() []const u8 {
    return "common/types/value/nested";
}

test "nested module path" {
    try std.testing.expectEqualStrings("common/types/value/nested", modulePath());
}

test "nested resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/types/value/nested"));
}
