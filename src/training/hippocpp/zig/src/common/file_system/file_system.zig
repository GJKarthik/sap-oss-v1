//! common/file_system/file_system parity module.
...
pub fn modulePath() []const u8 {
    return "common/file_system/file_system";
}

test "file_system module path" {
    try std.testing.expectEqualStrings("common/file_system/file_system", modulePath());
}

test "file_system resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/file_system/file_system"));
}
