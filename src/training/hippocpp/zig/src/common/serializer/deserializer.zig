//! common/serializer/deserializer parity module.
...
pub fn modulePath() []const u8 {
    return "common/serializer/deserializer";
}

test "deserializer module path" {
    try std.testing.expectEqualStrings("common/serializer/deserializer", modulePath());
}

test "deserializer resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/serializer/deserializer"));
}
