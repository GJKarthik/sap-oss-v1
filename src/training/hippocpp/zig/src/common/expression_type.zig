//! common/expression_type parity module.
...
pub fn modulePath() []const u8 {
    return "common/expression_type";
}

test "expression_type module path" {
    try std.testing.expectEqualStrings("common/expression_type", modulePath());
}

test "expression_type resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/expression_type"));
}
