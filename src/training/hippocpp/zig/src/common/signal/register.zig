//! common/signal/register parity module.
...
pub fn modulePath() []const u8 {
    return "common/signal/register";
}

test "register module path" {
    try std.testing.expectEqualStrings("common/signal/register", modulePath());
}

test "register resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/signal/register"));
}
