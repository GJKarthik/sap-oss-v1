...
pub const ExtensionInfo = struct {
    ...
    pub fn init(_: std.mem.Allocator, name: []const u8) ExtensionInfo {
        return .{
            .name = name,
...
pub const ExtensionRegistry = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) ExtensionRegistry {
        return .{
            .allocator = allocator,
            .extensions = .{ .unmanaged = .empty, .allocator = std.testing.allocator, .ctx = .{} },
            .search_paths = .{},
        };
    }
    ...
    pub fn listLoaded(self: *const ExtensionRegistry, _: std.mem.Allocator) !std.ArrayList([]const u8) {
        var result = .{};
...
pub const ExtensionManager = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) ExtensionManager {
        var manager = ExtensionManager{
            ...
            .builtin = .{ .unmanaged = .empty, .allocator = std.testing.allocator, .ctx = .{} },
        };
    ...
    /// List _all _extensions
    pub fn listExtensions(self: *const ExtensionManager, _: std.mem.Allocator) !std.ArrayList(ExtensionInfo) {
        var result = .{};
        errdefer result.deinit();
        
        var iter = self.registry.extensions.valueIterator();
        while (iter.next()) |ext| {
            try result.append(self.allocator, ext.info);
        }
...
pub const FunctionExtension = struct {
    ...
    pub fn deinit(self: *FunctionExtension) void {
        ...
        while (iter.next()) |func| {
            func.arg_types.deinit(self.base.allocator);
        }
...
test "extension registry" {
    ...
    const ext = Extension.init(allocator, "myext");
...
test "function extension" {
    ...
    try func.arg_types.append(std.testing.allocator, "INT64");
    try func.arg_types.append(std.testing.allocator, "INT64");
