//! parser/create_macro parity module.
...
pub fn modulePath() []const u8 {
    return "parser/create_macro";
}

test "create_macro module path" {
    try std.testing.expectEqualStrings("parser/create_macro", modulePath());
}

test "create_macro resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("parser/create_macro"));
}
