//! parser/transformer parity module.
...
test "transformer module path" {
    try std.testing.expectEqualStrings("parser/transformer", modulePath());
}

test "transformer resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("parser/transformer"));
}
