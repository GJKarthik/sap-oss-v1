...
pub const TransactionContext = struct {
    ...
    pub fn deinit(self: *Self) void {
        ...
        self.local_undo_buffer.deinit(self.allocator);
    ...
    pub fn addToWriteSet(self: *Self, entry: WriteSetEntry) !void {
        try self.write_set.append(self.allocator, entry);
    }
    ...
    pub fn createSavepoint(self: *Self, name: []const u8) !void {
        ...
        try self.savepoints.append(self.allocator, sp);
    }
...
// Wait-for graph edge for deadlock detection
    ...
    pub fn deinit(self: *Self) void {
        ...
        while (iter.next()) |locks| {
            locks.deinit(self.allocator);
        }
        self.resource_locks.deinit();
        self.wait_for_graph.deinit(self.allocator);
    }
    ...
    pub fn acquireLock(self: *Self, txn_id: TransactionID, resource_id: u64, lock_type: LockType) !bool {
        ...
        // Check for conflicts
        var blocking_txns: std.ArrayList(TransactionID) = .{};
        defer blocking_txns.deinit(self.allocator);
        ...
        for (result.value_ptr.items) |lock| {
            ...
            if (!self.locksCompatible(lock.lock_type, lock_type)) {
                try blocking_txns.append(self.allocator, lock.txn_id);
            }
        }
        ...
        if (blocking_txns.items.len > 0) {
            ...
            for (blocking_txns.items) |holding_txn| {
                try self.wait_for_graph.append(self.allocator, .{
                    .waiting_txn = txn_id,
        ...
        try result.value_ptr.append(self.allocator, request);
    ...
    fn detectDeadlock(self: *Self, start_txn: TransactionID) bool {
        ...
        var stack: std.ArrayList(TransactionID) = .{};
        defer stack.deinit(self.allocator);
        
        stack.append(self.allocator, start_txn) catch return false;
        ...
        while (stack.items.len > 0) {
            ...
            for (self.wait_for_graph.items) |edge| {
                if (edge.waiting_txn == current) {
                    stack.append(self.allocator, edge.holding_txn) catch continue;
                }
            }
        }
...
pub const TransactionManager = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !*Self {
        ...
        self.* = .{
            .allocator = allocator,
            .active_transactions = .{ .unmanaged = .empty, .allocator = self.allocator, .ctx = .{} },
            .next_txn_id = 1,
            .committed_ts = 0,
    ...
    pub fn acquireLock(self: *Self, txn: *TransactionContext, resource_id: u64, lock_type: LockType) !bool {
        ...
        if (granted) {
            ...
            request.granted = true;
            try txn.held_locks.append(self.allocator, request);
        } else {
            self.mutex.lock();
...
test "mvcc visibility" {
    ...
    const txn = try tm.beginTransaction(.READ_ONLY);
