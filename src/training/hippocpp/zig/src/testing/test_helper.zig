...
pub const MockRow = struct {
    ...
    pub fn addInt(self: *MockRow, val: i64) !void {
        try self.values.append(self.allocator, MockValue{ .int_val = val });
    }
    
    pub fn addString(self: *MockRow, val: []const u8) !void {
        try self.values.append(self.allocator, MockValue{ .string_val = val });
    }
    
    pub fn addBool(self: *MockRow, val: bool) !void {
        try self.values.append(self.allocator, MockValue{ .bool_val = val });
    }
    
    pub fn addNull(self: *MockRow) !void {
        try self.values.append(self.allocator, MockValue{ .null_val = {} });
    }
...
pub const MockTable = struct {
    ...
    pub fn addRow(self: *MockTable) !*MockRow {
        const row = MockRow.init(self.allocator);
...
pub const TestFixture = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) TestFixture {
        return .{
            .allocator = allocator,
            .tables = .{ .unmanaged = .empty, .allocator = std.testing.allocator, .ctx = .{} },
        };
    }
    ...
    pub fn createTable(self: *TestFixture, name: []const u8) !*MockTable {
        const table = MockTable.init(self.allocator, name);
