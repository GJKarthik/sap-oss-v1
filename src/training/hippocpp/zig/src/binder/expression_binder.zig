//! binder/expression_binder parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/expression_binder";
}

test "expression_binder binding path" {
    try std.testing.expectEqualStrings("binder/expression_binder", bindingPath());
}

test "expression_binder effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/expression_binder"));
}
