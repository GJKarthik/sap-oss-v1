//! binder/expression/expression_util parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/expression/expression_util";
}

test "expression_util binding path" {
    try std.testing.expectEqualStrings("binder/expression/expression_util", bindingPath());
}

test "expression_util effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/expression/expression_util"));
}
