//! binder/bound_scan_source parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bound_scan_source";
}

test "bound_scan_source binding path" {
    try std.testing.expectEqualStrings("binder/bound_scan_source", bindingPath());
}

test "bound_scan_source effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bound_scan_source"));
}
