//! binder/bind/read/bind_match parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/read/bind_match";
}

test "bind_match binding path" {
    try std.testing.expectEqualStrings("binder/bind/read/bind_match", bindingPath());
}

test "bind_match effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/read/bind_match"));
}
