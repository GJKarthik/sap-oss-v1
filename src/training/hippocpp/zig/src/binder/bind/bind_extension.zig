//! binder/bind/bind_extension parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_extension";
}

test "bind_extension binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_extension", bindingPath());
}

test "bind_extension effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_extension"));
}
