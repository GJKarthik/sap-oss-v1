...
const ALLOWED_HOSTS = /^(world-monitor\.app|[a-z0-9-]+\.world-monitor\.app|localhost(:[0-9]+)?|127\.0\.0\.1(:[0-9]+)?)$/;
const CANONICAL_HOST = 'world-monitor.app';

export default function handler(req, res) {
  const rawHost = req.headers.host || CANONICAL_HOST;
  const trustedHost = ALLOWED_HOSTS.test(rawHost) ? rawHost : CANONICAL_HOST;

  const url = new URL(req.url, `https://${trustedHost}`);
  ...
  const baseUrl = `https://${trustedHost}`;
