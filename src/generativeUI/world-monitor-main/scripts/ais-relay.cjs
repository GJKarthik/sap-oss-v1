  ...
  if (req.url === '/health' || req.url === '/') {
    ...
    try {
      ...
      const sendError = (statusCode, message) => {
        ...
        // Sanitize message: pass through only known safe static strings,
        // replace any system error details (ECONNREFUSED, DNS, etc.) with generic text.
        const safeMessage = typeof message === 'string' && /^[A-Za-z0-9 _:.-]{1,80}$/.test(message)
          ? message
          : 'Upstream request failed';
        res.writeHead(statusCode, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: safeMessage }));
        rejectInFlight(new Error(safeMessage));
      };
