...
embedded graph database source tree.  It is used by `ui5-webcomponents-ngx-main` for:

| Consumer | Purpose |
|----------|---------|
| `mcp-server/src/kuzu-store.ts` | TypeScript graph-RAG store (component co-usage graph, slot traversal, module membership) |
| `kuzu/mojo/kuzu_ffi.mojo` | Mojo FFI wrapper for high-performance Cypher queries from the Angular GenUI pipeline |
...
# 5. Re-run the Mojo FFI build (see mcp-server build tooling)
```
