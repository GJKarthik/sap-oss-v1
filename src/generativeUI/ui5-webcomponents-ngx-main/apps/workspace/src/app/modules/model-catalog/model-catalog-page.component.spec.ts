...
describe('ModelCatalogPageComponent', () => {
  it('loads models from live OpenAI endpoint when route is healthy', () => {
    ...
    const component = new ModelCatalogPageComponent(http, health);
  ...
  it('flags invalid model contract when response shape is unexpected', () => {
    ...
    expect(component.models).toEqual([]);
    expect(component.lastError).toContain('Unexpected model catalog response');
  });
