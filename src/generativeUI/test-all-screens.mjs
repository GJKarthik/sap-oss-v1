...
const TRAINING_BASE = 'http://localhost:4200';
const UI5_BASE = 'http://localhost:4201';
...
(async () => {
  ...
  console.log('=== Testing UI5 App (localhost:4201) ===');
  const ui5Results = await auditApp(browser, UI5_BASE, UI5_ROUTES, 'UI5');

  await browser.close();

  const all = [...trainingResults, ...ui5Results];
  let totalErrors = 0;
  let totalWarnings = 0;
