...
export interface VocabSearchResult { terms: VocabTerm[]; total: number; query: string; }
export interface VocabStatistics { total_vocabularies: number; total_terms: number; terms_by_vocabulary: Record<string, number>; }
export interface SchemaColumn { name: string; sql_type: string; nullable: boolean; annotations?: Record<string, unknown>; }
export interface SchemaTable { name: string; schema?: string; columns: SchemaColumn[]; }
export interface SparqlResult { bindings: Record<string, unknown>[]; columns: string[]; total: number; }
export interface AnalyticalResult { rows: Record<string, unknown>[]; columns: string[]; aggregation?: string; total: number; }

export interface GovernanceRule { id: string; name: string; rule_type: string; active: boolean; description?: string; updated_at?: string; }
export interface PendingApproval { id: string; tool: string; query: string; table_name: string; estimated_rows: number; status: string; requested_by?: string; created_at?: string; }
...
export class McpService {
  ...
  constructor() {
    // Start health polling immediately — Training app handles auth at route level
  ...
  fetchGovernanceRules(): Observable<{ rules: GovernanceRule[] }> {
    return this.withRequestTimeout(
      this.http.get<{ rules: GovernanceRule[] }>(`${environment.apiBaseUrl}/governance/rules`)
    );
  }

  createGovernanceRule(rule: Partial<GovernanceRule>): Observable<GovernanceRule> {
    return this.withRequestTimeout(
      this.http.post<GovernanceRule>(`${environment.apiBaseUrl}/governance/rules`, rule)
    );
  }

  updateGovernanceRule(ruleId: string, updates: Partial<GovernanceRule>): Observable<GovernanceRule> {
    return this.withRequestTimeout(
      this.http.patch<GovernanceRule>(`${environment.apiBaseUrl}/governance/rules/${ruleId}`, updates)
    );
  }

  deleteGovernanceRule(ruleId: string): Observable<void> {
    return this.withRequestTimeout(
      this.http.delete<void>(`${environment.apiBaseUrl}/governance/rules/${ruleId}`)
    );
  }
  ...
  runDataProfiling(tableName: string, sampleSize = 1000): Observable<unknown> {
    return this.withRequestTimeout(
      this.http.post<unknown>(`${environment.apiBaseUrl}/data-quality/profile`, { table_name: tableName, sample_size: sampleSize })
    );
  }

  detectAnomalies(tableName: string, column: string, method = 'zscore'): Observable<unknown> {
    return this.withRequestTimeout(
      this.http.post<unknown>(`${environment.apiBaseUrl}/data-quality/anomalies`, { table_name: tableName, column, method })
    );
  }

  fetchPendingApprovals(): Observable<PendingApproval[]> {
    return this.withRequestTimeout(
      this.http.get<PendingApproval[]>(`${environment.apiBaseUrl}/data-quality/approvals`)
    );
  }

  approveQuery(approvalId: string): Observable<void> {
    return this.withRequestTimeout(
      this.http.post<void>(`${environment.apiBaseUrl}/data-quality/approvals/${approvalId}/approve`, {})
    );
  }

  rejectQuery(approvalId: string): Observable<void> {
    return this.withRequestTimeout(
      this.http.post<void>(`${environment.apiBaseUrl}/data-quality/approvals/${approvalId}/reject`, {})
    );
  }

  submitApproval(entry: { tool: string; table_name: string; sql: string; estimated_rows: number }): Observable<void> {
    return this.withRequestTimeout(
      this.http.post<void>(`${environment.apiBaseUrl}/data-quality/approvals`, entry)
    );
  }

  // OData Vocabulary MCP
  listVocabularies(): Observable<{ vocabularies: string[] }> { return this.callTool<{ vocabularies: string[] }>(this.vocabUrl, 'list_vocabularies', {}); }
  getVocabulary(name: string): Observable<unknown> { return this.callTool<unknown>(this.vocabUrl, 'get_vocabulary', { name }); }
  searchVocabTerms(query: string, vocabulary?: string, limit = 20): Observable<VocabSearchResult> { const args: Record<string, unknown> = { query, limit }; if (vocabulary) { args['vocabulary'] = vocabulary; } return this.callTool<VocabSearchResult>(this.vocabUrl, 'search_terms', args); }
  vocabSemanticSearch(query: string, topK = 10, threshold = 0.5): Observable<VocabSearchResult> { return this.callTool<VocabSearchResult>(this.vocabUrl, 'semantic_search', { query, top_k: topK, threshold }); }
  getRagContext(query: string, entityType?: string): Observable<unknown> { const args: Record<string, unknown> = { query }; if (entityType) { args['entity_type'] = entityType; } return this.callTool<unknown>(this.vocabUrl, 'get_rag_context', args); }
  generateAnnotations(entityName: string, properties: string[]): Observable<{ annotations: string }> { return this.callTool<{ annotations: string }>(this.vocabUrl, 'generate_annotations', { entity_name: entityName, properties }); }
  validateAnnotations(annotations: string): Observable<{ valid: boolean; errors?: string[] }> { return this.callTool<{ valid: boolean; errors?: string[] }>(this.vocabUrl, 'validate_annotations', { annotations }); }
  getVocabStatistics(): Observable<VocabStatistics> { return this.callTool<VocabStatistics>(this.vocabUrl, 'get_statistics', {}); }

  // LangChain HANA MCP
  hanaVectorSearch(tableName: string, query: string, k = 4): Observable<{ documents: unknown[]; scores: number[] }> { return this.callTool<{ documents: unknown[]; scores: number[] }>(this.hanaUrl, 'vector_search', { table_name: tableName, query, k }); }
  hanaAddDocuments(tableName: string, documents: string[], metadatas?: Record<string, unknown>[]): Observable<{ documents_added: number }> { const args: Record<string, unknown> = { table_name: tableName, documents }; if (metadatas) { args['metadatas'] = metadatas; } return this.callTool<{ documents_added: number }>(this.hanaUrl, 'add_documents', args); }
  hanaCreateCollection(tableName: string, embeddingModel = 'default'): Observable<{ status: string; table_name: string }> { return this.callTool<{ status: string; table_name: string }>(this.hanaUrl, 'create_collection', { table_name: tableName, embedding_model: embeddingModel }); }
  hanaRagQuery(query: string, tableName: string, k = 4): Observable<RAGResult> { return this.callTool<RAGResult>(this.hanaUrl, 'rag_query', { query, table_name: tableName, k }); }
  hanaSplitText(text: string, chunkSize = 500, overlap = 50): Observable<{ chunks: string[] }> { return this.callTool<{ chunks: string[] }>(this.hanaUrl, 'split_text', { text, chunk_size: chunkSize, overlap }); }
  hanaRerank(query: string, documents: string[], topK = 5): Observable<{ ranked: Array<{ document: string; score: number }> }> { return this.callTool<{ ranked: Array<{ document: string; score: number }> }>(this.hanaUrl, 'rerank', { query, documents, top_k: topK }); }
  hanaListCollections(): Observable<{ collections: Array<{ table_name: string; document_count: number }> }> { return this.callTool<{ collections: Array<{ table_name: string; document_count: number }> }>(this.hanaUrl, 'list_collections', {}); }
  hanaListTables(): Observable<string[]> { return this.callTool<string[]>(this.hanaUrl, 'list_tables', {}); }
  hanaGetTableSchema(tableName: string): Observable<SchemaTable> { return this.callTool<SchemaTable>(this.hanaUrl, 'get_table_schema', { table_name: tableName }); }
  hanaSparqlQuery(query: string): Observable<SparqlResult> { return this.callTool<SparqlResult>(this.hanaUrl, 'sparql_query', { query }); }
  hanaAnalyticalQuery(calcView: string, dimensions: string[], measures: string[], aggregation = 'SUM'): Observable<AnalyticalResult> { return this.callTool<AnalyticalResult>(this.hanaUrl, 'analytical_query', { calc_view: calcView, dimensions, measures, aggregation }); }
  hanaListCalcViews(): Observable<{ calc_views: string[] }> { return this.callTool<{ calc_views: string[] }>(this.hanaUrl, 'list_calc_views', {}); }
}
