//! Zero-CPU-Touch Fused Async GPU Pipeline for AIPrompt Streaming
...
//!   2. JSON Parser: Locate "input"/"prompt" field via Metal `json_find_key` kernel
...
/// Context for all Metal GPU dispatch callbacks.
const MetalGpuDispatchContext = struct {
    ...
    fn dispatchParse(
        ...
        const data_buf = metal_bindings.createBufferWithBytes(
            device, raw_json, metal_bindings.MTLResourceStorageModeShared,
        ) orelse return false;
        ...
        const dispatched = lib.dispatchComputeWithBytes(
            .json_find_key,
            &.{
                .{ .buf = data_buf, .index = 0 },
                .{ .buf = result_buf, .index = 1 },
            },
            &.{
        ...
        if (metal_bindings.getBufferContents(result_buf)) |ptr| {
            ...
            return r[2] == @intFromEnum(json_parser.ParseStatus.found);
        }
        return false;
    }
    ...
    fn dispatchTokenize(
        ...
        const text_buf = metal_bindings.createBufferWithBytes(
            device, text, metal_bindings.MTLResourceStorageModeShared,
        ) orelse return false;
        ...
        const dispatched = lib.dispatchComputeWithBytes(
            .gpu_tokenize_bytes,
            &.{
                .{ .buf = text_buf, .index = 0 },
                .{ .buf = tok_buf, .index = 1 },
                .{ .buf = cnt_buf, .index = 2 },
            },
            &.{
                .{ .ptr = @ptrCast(&text_len), .len = @sizeOf(u32), .index = 3 },
                .{ .ptr = @ptrCast(&max_tok), .len = @sizeOf(u32), .index = 4 },
            },
    ...
    fn dispatchEmbed(ctx_ptr: *anyopaque, tokens: []const u32, output: []f32, embed_dim: usize) bool {
        ...
        const token_buf = metal_bindings.createBufferWithBytes(
            device, std.mem.sliceAsBytes(tokens), metal_bindings.MTLResourceStorageModeShared,
        ) orelse return false;
        ...
        if (metal_bindings.getBufferContents(table_buf)) |ptr| {
            ...
            for (0..vocab_size) |tok| {
                ...
                for (0..embed_dim) |d| {
                    ...
                    table[tok * embed_dim + d] = (norm * 2.0) - 1.0;
                }
            }
        } else return false;
        ...
        const dispatched = lib.dispatchComputeWithBytes(
            .embedding_lookup,
            &.{
                .{ .buf = token_buf, .index = 0 },
                .{ .buf = table_buf, .index = 1 },
                .{ .buf = out_buf, .index = 2 },
            },
            &.{
...
pub const ZeroCopyPipeline = struct {
    ...
    /// Creates a MetalGpuDispatchContext and wires all three stage callbacks
    /// (JSON parse, tokenize, embed) into the fusion pipeline.
