...
The MCP server exposes AI Core tools (`ai_core_chat`, `ai_core_embed`, `hana_vector_search`, `list_deployments`), graph tools (`kuzu_index`, `kuzu_query`), `mangle_query` (in-memory fact store plus derived predicates), and `orchestration_run`. It also exposes Colored Petri Net (CPN) workflow tools: `cpn_load`, `cpn_reset`, `cpn_marking`, `cpn_fire`, and `cpn_step`. It exposes three resources: `deployment://list` (real-time deployment list), `mangle://facts` (current Mangle fact store), and `mangle://rules` (loaded Mangle reasoning rules). Two prompt templates are registered: `rag_query` and `data_analysis`.

**Colored Petri Net engine.** The package includes an in-process CPN runtime (`packages/mcp-server/src/cpn/`): places hold multisets of JSON-serializable colored tokens; transitions consume and produce tokens with optional guards over merged input payloads (`path` + `eq` / `neq`). Built-in scenario `odps_close_process` models ODPS close-process stages **S01→S02→S03** (aligned with `petri_stage` checks in `langchain-integration-for-sap-hana-cloud-main/mangle/domain/odps_compliance.mg`). When remote MCP federation does not handle a scenario, `orchestration_run` executes this net, returns a firing trace and final marking, and updates `petri_stage` facts (`{ app, stage }` rows, with cumulative linear stages for compliance). `mangle_query` answers `petri_stage`, `cpn_enabled`, and `cpn_marking_snapshot` from the same store. Multi-model LLM orchestration remains the responsibility of the external `@sap-ai-sdk/orchestration` service when configured separately; the local path is explicitly a Petri-net workflow engine, not a replacement for SAP AI Core orchestration APIs.

The server integrates with the local Mangle rule files at `../../mangle/`, specifically `a2a/mcp.mg` (service registry and routing), `connectors/aicore.mg` (AI Core deployment rules), and `standard/rules.mg` (audit, health, and quality rules). Configuration is entirely via environment variables: `AICORE_CLIENT_ID`, `AICORE_CLIENT_SECRET`, `AICORE_AUTH_URL`, `AICORE_BASE_URL`, `AICORE_RESOURCE_GROUP`, and `MCP_PORT` (default 9090). The server depends on `@modelcontextprotocol/sdk` v1.0.0, `express`, `ws` (WebSocket), `dotenv`, `uuid`, and `kuzu`. Federated MCP endpoints listed in `AI_SDK_REMOTE_MCP_ENDPOINTS` exclude the server’s own URL (same host/port as `MCP_PORT`) to avoid JSON-RPC self-calls.
...
The agent is composed of two classes. The `MangleEngine` class simulates a Mangle Datalog inference engine by loading facts into a Python dictionary and exposing a `query(predicate, *args)` interface. The predicates implemented are `route_to_vllm`, `route_to_aicore`, `requires_human_review`, `safety_check_passed`, `agent_can_use`, `get_prompting_policy`, and `autonomy_level`. Workflow stage facts such as `petri_stage/2` are **not** replicated in this stub; use the MCP server’s `mangle_query` / `orchestration_run` (built-in CPN) when ODPS Petri compliance data is required. The routing decision is based on a keyword set of confidential financial terms: `trading`, `position`, `pnl`, `profit`, `loss`, `balance`, `fx`, `derivative`, `swap`, `hedge`, `var`, `exposure`, `counterparty`, `risk`, `liquidity`, and `capital`. If any of these appear in the prompt (case-insensitive), the request is routed to the vLLM on-premise backend; otherwise it goes to AI Core. This keyword list is domain-specific to financial services use cases.
