...
function streamPost(baseUrl, body, { timeoutMs = 10000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(options, (res) => { // lgtm[js/clear-text-transmission-of-sensitive-data]
      let raw = "";
...
describe("E2E Streaming — Abort Mid-Stream", () => {
  ...
  test("client can close connection mid-stream without server error", async () => {
    ...
    await new Promise((resolve) => {
      const req = http.request( // lgtm[js/clear-text-transmission-of-sensitive-data]
        {
          hostname: url.hostname,
          port: Number(url.port),
          path: url.pathname,
