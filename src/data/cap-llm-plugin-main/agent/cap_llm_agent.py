"""
CAP LLM Agent with ODPS + Regulations Integration

Provides governance-aware LLM routing for CAP applications:
- Confidential business/financial data → vLLM (on-premise)
...
class MangleEngine:
    ...
    def _load_rules(self):
        ...
        self.facts["agent_requires_approval"] = {
            "update_vector_store", "delete_embeddings", "modify_rag_config"
        }
    ...
    def query(self, predicate: str, *args) -> List[Dict]:
        ...
        if predicate == "get_prompting_policy":
            product_id = args[0] if args else "cap-llm-service-v1"
...
class CapLlmAgent:
    """
    CAP LLM Agent with ODPS data product awareness and regulations compliance.
    """
    ...
    async def invoke(self, prompt: str, context: Optional[Dict] = None) -> Dict[str, Any]:
        context = context or {}
        tool = context.get("tool", "cap_chat")
        ...
        prompting = self.mangle.query("get_prompting_policy", "cap-llm-service-v1")
    ...
    def check_governance(self, prompt: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "prompting_policy": prompting[0] if prompting else {},
            "data_product": "cap-llm-service-v1"
        }
...
def main():
    ...
    print("=" * 60)
    print("CAP LLM Agent - Governance-Aware LLM Routing")
    print("=" * 60)
    
    # Test 1: General query
    print("\n--- Test 1: General Query ---")
    governance = agent.check_governance("How do I create a CAP service?")
    print(f"Routing: {governance['routing']['backend']}")
    print(f"Reason: {governance['routing']['reason']}")
    
    # Test 2: Business data (customer)
    print("\n--- Test 2: Customer Data Query ---")
    governance = agent.check_governance("Show me customer orders for this month")
    print(f"Routing: {governance['routing']['backend']}")
    print(f"Reason: {governance['routing']['reason']}")
    
    # Test 3: Financial data
    print("\n--- Test 3: Financial Data Query ---")
    governance = agent.check_governance("What is the revenue forecast for Q4?")
    print(f"Routing: {governance['routing']['backend']}")
    print(f"Reason: {governance['routing']['reason']}")
