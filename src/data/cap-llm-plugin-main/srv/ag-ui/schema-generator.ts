...
import type { A2UiSchema, A2UiComponent } from './event-types';
...
export const UI_GENERATION_SYSTEM_PROMPT = `You are an expert SAP Fiori UI architect. Your task is to generate A2UiSchema JSON structures that describe enterprise UIs using UI5 Web Components.
...
export class SchemaGenerator {
  ...
  async generateSchema(
    ...
    llmPlugin: { getChatCompletionWithConfig: (config: unknown, payload: unknown) => Promise<unknown> }
    ...
    const response = await llmPlugin.getChatCompletionWithConfig(
      ...
      {
        messages,
        functions: [GENERATE_UI_FUNCTION],
   ...
   */
  async *generateSchemaStreaming(
    params: GenerateSchemaParams,
    llmPlugin: { streamChatCompletion: (params: unknown, req?: unknown) => Promise<string> }
   ...
   */
  private buildMessages(params: GenerateSchemaParams): Array<{ role: string; content: string }> {
   ...
   */
  private parseResponse(response: unknown): GenerateSchemaResult {
    const resp = response as {
      getContent?: () => string;
      choices?: Array<{ message?: { function_call?: { arguments?: string }; content?: string } }>;
    };
   ...
   */
  private parseSchemaFromContent(content: string): A2UiSchema {
   ...
   */
  private sanitizeSchema(schema: unknown): A2UiSchema {
    const s = schema as A2UiSchema;
 ...
 */
export function createTextSchema(text: string, id = 'text-message'): A2UiSchema {
  return {
    $schema: 'https://sap.github.io/ui5-webcomponents/schemas/a2ui-schema.json',
    version: '1.0',
    layout: {
 ...
 */
export function createLoadingSchema(message = 'Loading...'): A2UiSchema {
  return {
    $schema: 'https://sap.github.io/ui5-webcomponents/schemas/a2ui-schema.json',
    version: '1.0',
    layout: {
 ...
 */
export function createErrorSchema(error: string, title = 'Error'): A2UiSchema {
  return {
    $schema: 'https://sap.github.io/ui5-webcomponents/schemas/a2ui-schema.json',
    version: '1.0',
    layout: {
