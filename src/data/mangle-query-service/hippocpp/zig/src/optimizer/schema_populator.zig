//! optimizer/schema_populator parity module.
...
pub fn modulePath() []const u8 {
    return "optimizer/schema_populator";
}

test "schema_populator module path" {
    try std.testing.expectEqualStrings("optimizer/schema_populator", modulePath());
}

test "schema_populator resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("optimizer/schema_populator"));
}
