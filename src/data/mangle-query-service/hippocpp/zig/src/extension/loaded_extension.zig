//! extension/loaded_extension parity module.
...
pub fn modulePath() []const u8 {
    return "extension/loaded_extension";
}

test "loaded_extension module path" {
    try std.testing.expectEqualStrings("extension/loaded_extension", modulePath());
}

test "loaded_extension resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("extension/loaded_extension"));
}
