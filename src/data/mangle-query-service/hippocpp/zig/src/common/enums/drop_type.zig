//! common/enums/drop_type parity module.
...
pub fn modulePath() []const u8 {
    return "common/enums/drop_type";
}

test "drop_type module path" {
    try std.testing.expectEqualStrings("common/enums/drop_type", modulePath());
}

test "drop_type resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/enums/drop_type"));
}
