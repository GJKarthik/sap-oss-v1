//! common/enums/path_semantic parity module.
...
pub fn modulePath() []const u8 {
    return "common/enums/path_semantic";
}

test "path_semantic module path" {
    try std.testing.expectEqualStrings("common/enums/path_semantic", modulePath());
}

test "path_semantic resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/enums/path_semantic"));
}
