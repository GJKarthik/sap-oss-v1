//! common/enums/scan_source_type parity module.
...
pub fn modulePath() []const u8 {
    return "common/enums/scan_source_type";
}

test "scan_source_type module path" {
    try std.testing.expectEqualStrings("common/enums/scan_source_type", modulePath());
}

test "scan_source_type resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/enums/scan_source_type"));
}
