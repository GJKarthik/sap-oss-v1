//! common/enums/rel_direction parity module.
...
pub fn modulePath() []const u8 {
    return "common/enums/rel_direction";
}

test "rel_direction module path" {
    try std.testing.expectEqualStrings("common/enums/rel_direction", modulePath());
}

test "rel_direction resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/enums/rel_direction"));
}
