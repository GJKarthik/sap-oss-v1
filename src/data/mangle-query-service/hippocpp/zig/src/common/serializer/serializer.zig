//! common/serializer/serializer parity module.
...
pub fn modulePath() []const u8 {
    return "common/serializer/serializer";
}

test "serializer module path" {
    try std.testing.expectEqualStrings("common/serializer/serializer", modulePath());
}

test "serializer resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/serializer/serializer"));
}
