//! common/serializer/buffer_writer parity module.
...
pub fn modulePath() []const u8 {
    return "common/serializer/buffer_writer";
}

test "buffer_writer module path" {
    try std.testing.expectEqualStrings("common/serializer/buffer_writer", modulePath());
}

test "buffer_writer resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/serializer/buffer_writer"));
}
