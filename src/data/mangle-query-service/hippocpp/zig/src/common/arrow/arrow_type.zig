//! common/arrow/arrow_type parity module.
...
pub fn modulePath() []const u8 {
    return "common/arrow/arrow_type";
}

test "arrow_type module path" {
    try std.testing.expectEqualStrings("common/arrow/arrow_type", modulePath());
}

test "arrow_type resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/arrow/arrow_type"));
}
