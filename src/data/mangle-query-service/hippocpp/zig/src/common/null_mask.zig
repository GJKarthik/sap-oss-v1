//! common/null_mask parity module.
...
pub fn modulePath() []const u8 {
    return "common/null_mask";
}

test "null_mask module path" {
    try std.testing.expectEqualStrings("common/null_mask", modulePath());
}

test "null_mask resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/null_mask"));
}
