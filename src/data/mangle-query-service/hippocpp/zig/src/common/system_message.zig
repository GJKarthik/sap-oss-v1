//! common/system_message parity module.
...
pub fn modulePath() []const u8 {
    return "common/system_message";
}

test "system_message module path" {
    try std.testing.expectEqualStrings("common/system_message", modulePath());
}

test "system_message resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/system_message"));
}
