...
test "checksum module path" {
    try std.testing.expectEqualStrings("common/checksum", modulePath());
}

test "checksum resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/checksum"));
}
