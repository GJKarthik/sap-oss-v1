//! common/data_chunk/sel_vector parity module.
...
pub fn modulePath() []const u8 {
    return "common/data_chunk/sel_vector";
}

test "sel_vector module path" {
    try std.testing.expectEqualStrings("common/data_chunk/sel_vector", modulePath());
}

test "sel_vector resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/data_chunk/sel_vector"));
}
