//! common/windows_utils parity module.
...
pub fn modulePath() []const u8 {
    return "common/windows_utils";
}

test "windows_utils module path" {
    try std.testing.expectEqualStrings("common/windows_utils", modulePath());
}

test "windows_utils resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/windows_utils"));
}
