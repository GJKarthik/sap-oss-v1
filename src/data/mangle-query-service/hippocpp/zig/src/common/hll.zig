...
test "hll module path" {
    try std.testing.expectEqualStrings("common/hll", modulePath());
}

test "hll resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/hll"));
}
