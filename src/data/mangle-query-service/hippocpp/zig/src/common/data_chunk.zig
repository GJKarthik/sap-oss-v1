...
pub const DataChunk = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, capacity: usize) DataChunk {
        return .{
            .allocator = allocator,
            .columns = std.ArrayList(ValueVector).init(allocator),
            .selection = null,
            .capacity = capacity,
        };
    }
    ...
    pub fn addColumn(self: *DataChunk, data_type: ValueVector.DataType) !*ValueVector {
        var vec = try ValueVector.init(self.allocator, data_type, self.capacity);
        try self.columns.append(vec);
...
test "data chunk" {
    ...
    const col1 = try chunk.addColumn(.INT64);
    const col2 = try chunk.addColumn(.DOUBLE);
    
    col1.setInt64(0, 1);
    col1.setInt64(1, 2);
    col2.setDouble(0, 1.5);
    col2.setDouble(1, 2.5);
