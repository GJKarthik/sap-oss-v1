//! common/types/timestamp_t parity module.
...
pub fn modulePath() []const u8 {
    return "common/types/timestamp_t";
}

test "timestamp_t module path" {
    try std.testing.expectEqualStrings("common/types/timestamp_t", modulePath());
}

test "timestamp_t resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/types/timestamp_t"));
}
