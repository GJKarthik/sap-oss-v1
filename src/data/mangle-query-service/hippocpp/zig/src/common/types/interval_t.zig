//! common/types/interval_t parity module.
...
pub fn modulePath() []const u8 {
    return "common/types/interval_t";
}

test "interval_t module path" {
    try std.testing.expectEqualStrings("common/types/interval_t", modulePath());
}

test "interval_t resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/types/interval_t"));
}
