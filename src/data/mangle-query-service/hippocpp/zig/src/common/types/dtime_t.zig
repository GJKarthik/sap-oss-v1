//! common/types/dtime_t parity module.
...
pub fn modulePath() []const u8 {
    return "common/types/dtime_t";
}

test "dtime_t module path" {
    try std.testing.expectEqualStrings("common/types/dtime_t", modulePath());
}

test "dtime_t resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/types/dtime_t"));
}
