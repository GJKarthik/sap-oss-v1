//! common/types/uint128_t parity module.
...
pub fn modulePath() []const u8 {
    return "common/types/uint128_t";
}

test "uint128_t module path" {
    try std.testing.expectEqualStrings("common/types/uint128_t", modulePath());
}

test "uint128_t resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/types/uint128_t"));
}
