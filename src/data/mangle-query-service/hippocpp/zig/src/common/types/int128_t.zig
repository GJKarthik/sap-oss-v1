//! common/types/int128_t parity module.
...
pub fn modulePath() []const u8 {
    return "common/types/int128_t";
}

test "int128_t module path" {
    try std.testing.expectEqualStrings("common/types/int128_t", modulePath());
}

test "int128_t resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/types/int128_t"));
}
