//! common/type_utils parity module.
...
pub fn modulePath() []const u8 {
    return "common/type_utils";
}

test "type_utils module path" {
    try std.testing.expectEqualStrings("common/type_utils", modulePath());
}

test "type_utils resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/type_utils"));
}
