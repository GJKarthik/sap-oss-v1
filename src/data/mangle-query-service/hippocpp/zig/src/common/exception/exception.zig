//! common/exception/exception parity module.
...
pub fn modulePath() []const u8 {
    return "common/exception/exception";
}

test "exception module path" {
    try std.testing.expectEqualStrings("common/exception/exception", modulePath());
}

test "exception resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("common/exception/exception"));
}
