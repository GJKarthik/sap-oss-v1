...
pub const ValueVector = struct {
    ...
    fn getBytesPerValue(data_type: LogicalType) u64 {
        return switch (data_type) {
            ...
            .INTERNAL_ID => 16, // table_id + offset
            else => 8, // default
        };
    }
...
pub const DataChunk = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            ...
            .value_vectors = std.ArrayList(*ValueVector).init(allocator),
        };
    }
...
pub const ExpressionEvaluator = struct {
    ...
    pub fn init(
        ...
        return .{
            ...
            .result_vector = null,
            .is_result_flat = true,
            .children = std.ArrayList(*ExpressionEvaluator).init(allocator),
...
pub const CaseEvaluator = struct {
    ...
    pub fn create(allocator: std.mem.Allocator, return_type: LogicalType) !*Self {
        ...
        self.* = .{
            ...
            .when_evaluators = std.ArrayList(*ExpressionEvaluator).init(allocator),
            .then_evaluators = std.ArrayList(*ExpressionEvaluator).init(allocator),
            .else_evaluator = null,
        };
        return self;
    }
    ...
    pub fn addWhenThen(self: *Self, when_eval: *ExpressionEvaluator, then_eval: *ExpressionEvaluator) !void {
        try self.when_evaluators.append(when_eval);
        try self.then_evaluators.append(then_eval);
    }
    ...
    fn destroyImpl(base: *ExpressionEvaluator) void {
        const self: *Self = @fieldParentPtr("base", base);
        self.when_evaluators.deinit();
        self.then_evaluators.deinit();
        if (base.result_vector) |rv| {
            rv.deinit();
            base.allocator.destroy(rv);
        }
