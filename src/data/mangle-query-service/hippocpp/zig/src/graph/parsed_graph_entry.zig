//! graph/parsed_graph_entry parity module.
...
pub fn modulePath() []const u8 {
    return "graph/parsed_graph_entry";
}

test "parsed_graph_entry module path" {
    try std.testing.expectEqualStrings("graph/parsed_graph_entry", modulePath());
}

test "parsed_graph_entry resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("graph/parsed_graph_entry"));
}
