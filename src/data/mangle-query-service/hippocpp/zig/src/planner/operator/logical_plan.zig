//! planner/operator/logical_plan parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/operator/logical_plan";
}

test "logical_plan planner path" {
    try std.testing.expectEqualStrings("planner/operator/logical_plan", plannerPath());
}

test "logical_plan planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/operator/logical_plan"));
}
