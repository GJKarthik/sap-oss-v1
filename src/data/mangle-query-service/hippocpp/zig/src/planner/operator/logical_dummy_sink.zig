//! planner/operator/logical_dummy_sink parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/operator/logical_dummy_sink";
}

test "logical_dummy_sink planner path" {
    try std.testing.expectEqualStrings("planner/operator/logical_dummy_sink", plannerPath());
}

test "logical_dummy_sink planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/operator/logical_dummy_sink"));
}
