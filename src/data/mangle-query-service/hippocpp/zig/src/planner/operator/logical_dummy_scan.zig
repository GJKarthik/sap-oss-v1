//! planner/operator/logical_dummy_scan parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/operator/logical_dummy_scan";
}

test "logical_dummy_scan planner path" {
    try std.testing.expectEqualStrings("planner/operator/logical_dummy_scan", plannerPath());
}

test "logical_dummy_scan planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/operator/logical_dummy_scan"));
}
