//! planner/operator/logical_operator parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/operator/logical_operator";
}

test "logical_operator planner path" {
    try std.testing.expectEqualStrings("planner/operator/logical_operator", plannerPath());
}

test "logical_operator planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/operator/logical_operator"));
}
