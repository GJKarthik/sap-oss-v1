//! planner/subplans_table parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/subplans_table";
}

test "subplans_table planner path" {
    try std.testing.expectEqualStrings("planner/subplans_table", plannerPath());
}

test "subplans_table planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/subplans_table"));
}
