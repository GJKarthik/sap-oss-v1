...
pub const CardinalityEstimator = struct {
    ...
    node_id_name_to_dom: std.StringHashMap(u64),
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            ...
            .node_id_name_to_dom = std.StringHashMap(u64).init(allocator),
        };
    }
    ...
    pub fn initWithTable(self: *Self, table_id: u64, name: []const u8, row_count: u64) !void {
        ...
        try self.node_id_name_to_dom.put(name, row_count);
    }
    ...
    pub fn getNodeIDDom(self: *const Self, node_id_name: []const u8) u64 {
        if (self.node_id_name_to_dom.get(node_id_name)) |dom| {
            return atLeastOne(dom);
        }
    ...
    pub fn rectifyCardinality(self: *Self, node_id_name: []const u8, card: u64) void {
        if (self.node_id_name_to_dom.get(node_id_name)) |current| {
            const new_card = @min(current, card);
            self.node_id_name_to_dom.put(node_id_name, new_card) catch {};
        }
    }
    ...
    pub fn estimateFilter(self: *const Self, input_card: u64, predicate_type: PredicateType) u64 {
        const selectivity = switch (predicate_type) {
            .EQUALITY => PlannerKnobs.EQUALITY_PREDICATE_SELECTIVITY,
            .INEQUALITY => PlannerKnobs.NON_EQUALITY_PREDIVITY_SELECTIVITY,
        ...
        return atLeastOne(@intFromFloat(@as(f64, @floatFromInt(input_card)) * selectivity));
    }
    ...
    pub fn estimateAggregate(self: *const Self, child_card: u64, group_keys: []const []const u8) u64 {
        ...
        if (group_keys.len > 1) {
            ...
            estimated_groups = atLeastOne(@intFromFloat(@as(f64, @floatFromInt(estimated_groups)) * correlation_factor));
        }
    ...
    pub fn estimateIntersect(self: *const Self, join_node_ids: []const []const u8, probe_card: u64, build_cards: []const u64) u64 {
        ...
        const est_card_1 = @intFromFloat(@as(f64, @floatFromInt(probe_card)) * PlannerKnobs.NON_EQUALITY_PREDICATE_SELECTIVITY);
    ...
    pub fn estimateFlatten(_: *const Self, child_card: u64, multiplier: f64) u64 {
        return atLeastOne(@intFromFloat(@as(f64, @floatFromInt(child_card)) * multiplier));
    }
    
    /// Multiply cardinality by extension rate
    pub fn multiply(_: *const Self, extension_rate: f64, card: u64) u64 {
        return atLeastOne(@intFromFloat(extension_rate * @as(f64, @floatFromInt(card))));
    }
};
...
pub const QueryGraphPlanningInfo = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            .allocator = allocator,
            .correlated_exprs = std.ArrayList([]const u8).init(allocator),
        };
    }
    
    pub fn deinit(self: *Self) void {
        self.correlated_exprs.deinit();
    }
    ...
    pub fn addCorrExpr(self: *Self, expr_name: []const u8) !void {
        try self.correlated_exprs.append(expr_name);
    }
};
...
pub const JoinOrderEnumeratorContext = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            .allocator = allocator,
            .subplans = std.StringHashMap(LogicalPlan).init(allocator),
            .current_level = 0,
        };
    }
...
pub const QueryPlanner = struct {
    ...
    fn planUnionQuery(self: *Self, bound_stmt: *const BoundStatement) !LogicalPlan {
        var children_plans = std.ArrayList(LogicalPlan).init(self.allocator);
        defer children_plans.deinit();
        
        // Plan the first query
        try children_plans.append(try self.planSingleQuery(bound_stmt));
        
        // Plan union parts
        for (bound_stmt.union_queries.items) |union_stmt| {
            try children_plans.append(try self.planSingleQuery(union_stmt));
        }
    ...
    fn estimatePlan(self: *Self, plan: *LogicalPlan) void {
        if (plan.root) |root| {
            ...
            plan.estimated_cost = @intFromFloat(self.computeOperatorCost(root));
        }
    }
...
pub const PlanOptimizer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        var rules = std.EnumSet(OptimizationRule).initFull();
        return .{
            .allocator = allocator,
            .enabled_rules = rules,
            .max_iterations = 10,
