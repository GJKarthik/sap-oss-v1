//! planner/plan/plan_port_db parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/plan_port_db";
}

test "plan_port_db planner path" {
    try std.testing.expectEqualStrings("planner/plan/plan_port_db", plannerPath());
}

test "plan_port_db planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/plan_port_db"));
}
