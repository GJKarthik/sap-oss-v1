//! planner/plan/plan_join_order parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/plan_join_order";
}

test "plan_join_order planner path" {
    try std.testing.expectEqualStrings("planner/plan/plan_join_order", plannerPath());
}

test "plan_join_order planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/plan_join_order"));
}
