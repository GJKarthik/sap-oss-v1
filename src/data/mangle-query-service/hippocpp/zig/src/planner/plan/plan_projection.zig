//! planner/plan/plan_projection parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/plan_projection";
}

test "plan_projection planner path" {
    try std.testing.expectEqualStrings("planner/plan/plan_projection", plannerPath());
}

test "plan_projection planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/plan_projection"));
}
