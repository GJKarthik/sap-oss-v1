//! planner/plan/plan_node_scan parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/plan_node_scan";
}

test "plan_node_scan planner path" {
    try std.testing.expectEqualStrings("planner/plan/plan_node_scan", plannerPath());
}

test "plan_node_scan planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/plan_node_scan"));
}
