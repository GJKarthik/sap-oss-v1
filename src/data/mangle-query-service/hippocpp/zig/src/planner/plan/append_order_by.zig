//! planner/plan/append_order_by parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_order_by";
}

test "append_order_by planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_order_by", plannerPath());
}

test "append_order_by planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_order_by"));
}
