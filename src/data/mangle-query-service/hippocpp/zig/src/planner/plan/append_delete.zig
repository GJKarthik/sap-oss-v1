//! planner/plan/append_delete parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_delete";
}

test "append_delete planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_delete", plannerPath());
}

test "append_delete planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_delete"));
}
