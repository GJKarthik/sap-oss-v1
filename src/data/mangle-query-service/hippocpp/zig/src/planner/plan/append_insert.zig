//! planner/plan/append_insert parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_insert";
}

test "append_insert planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_insert", plannerPath());
}

test "append_insert planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_insert"));
}
