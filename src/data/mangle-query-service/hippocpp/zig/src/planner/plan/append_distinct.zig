//! planner/plan/append_distinct parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_distinct";
}

test "append_distinct planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_distinct", plannerPath());
}

test "append_distinct planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_distinct"));
}
