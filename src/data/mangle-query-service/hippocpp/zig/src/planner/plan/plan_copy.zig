//! planner/plan/plan_copy parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/plan_copy";
}

test "plan_copy planner path" {
    try std.testing.expectEqualStrings("planner/plan/plan_copy", plannerPath());
}

test "plan_copy planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/plan_copy"));
}
