//! planner/plan/append_empty_result parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_empty_result";
}

test "append_empty_result planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_empty_result", plannerPath());
}

test "append_empty_result planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_empty_result"));
}
