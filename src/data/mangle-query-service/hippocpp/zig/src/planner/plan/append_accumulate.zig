//! planner/plan/append_accumulate parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_accumulate";
}

test "append_accumulate planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_accumulate", plannerPath());
}

test "append_accumulate planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_accumulate"));
}
