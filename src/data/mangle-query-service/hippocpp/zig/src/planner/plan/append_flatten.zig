//! planner/plan/append_flatten parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_flatten";
}

test "append_flatten planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_flatten", plannerPath());
}

test "append_flatten planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_flatten"));
}
