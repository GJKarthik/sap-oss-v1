//! planner/plan/append_set parity module.
...
pub fn plannerPath() []const u8 {
    return "planner/plan/append_set";
}

test "append_set planner path" {
    try std.testing.expectEqualStrings("planner/plan/append_set", plannerPath());
}

test "append_set planner id fallback" {
    var stage = PlannerStage.init("");
    try std.testing.expect(stage.idMatches("planner/plan/append_set"));
}
