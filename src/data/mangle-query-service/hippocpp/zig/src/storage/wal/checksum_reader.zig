//! storage/wal/checksum_reader parity module.
...
pub fn modulePath() []const u8 {
    return "storage/wal/checksum_reader";
}

test "checksum_reader module path" {
    try std.testing.expectEqualStrings("storage/wal/checksum_reader", modulePath());
}

test "checksum_reader resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/wal/checksum_reader"));
}
