//! storage/wal/checksum_writer parity module.
...
pub fn modulePath() []const u8 {
    return "storage/wal/checksum_writer";
}

test "checksum_writer module path" {
    try std.testing.expectEqualStrings("storage/wal/checksum_writer", modulePath());
}

test "checksum_writer resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/wal/checksum_writer"));
}
