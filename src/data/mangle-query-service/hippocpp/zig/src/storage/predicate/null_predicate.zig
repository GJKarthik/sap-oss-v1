//! storage/predicate/null_predicate parity module.
...
pub fn modulePath() []const u8 {
    return "storage/predicate/null_predicate";
}

test "null_predicate module path" {
    try std.testing.expectEqualStrings("storage/predicate/null_predicate", modulePath());
}

test "null_predicate resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/predicate/null_predicate"));
}
