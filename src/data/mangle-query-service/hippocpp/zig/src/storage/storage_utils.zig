//! storage/storage_utils parity module.
...
pub fn modulePath() []const u8 {
    return "storage/storage_utils";
}

test "storage_utils module path" {
    try std.testing.expectEqualStrings("storage/storage_utils", modulePath());
}

test "storage_utils resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/storage_utils"));
}
