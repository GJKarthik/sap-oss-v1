//! storage/file_db_id_utils parity module.
...
pub fn modulePath() []const u8 {
    return "storage/file_db_id_utils";
}

test "file_db_id_utils module path" {
    try std.testing.expectEqualStrings("storage/file_db_id_utils", modulePath());
}

test "file_db_id_utils resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/file_db_id_utils"));
}
