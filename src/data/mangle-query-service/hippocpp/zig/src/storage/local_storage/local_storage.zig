...
pub const LocalNodeTable = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, table_id: TableID) Self {
        return .{
            .allocator = allocator,
            .table_id = table_id,
            .inserted_rows = std.ArrayList(RowOffset).init(allocator),
            .deleted_rows = std.AutoHashMap(RowOffset, void).init(allocator),
            .updated_columns = std.AutoHashMap(u32, LocalColumnChunk).init(allocator),
            .next_local_offset = 0,
        };
    }
    ...
    pub fn insert(self: *Self) !RowOffset {
        ...
        try self.inserted_rows.append(offset);
        return offset;
    }
...
pub const LocalRelTable = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, table_id: TableID) Self {
        return .{
            .allocator = allocator,
            .table_id = table_id,
            .inserted_rels = std.ArrayList(LocalRelEntry).init(allocator),
            .deleted_rels = std.AutoHashMap(u64, void).init(allocator),
            .next_local_rel_id = 0,
        };
    }
    
    pub fn deinit(self: *Self) void {
        self.inserted_rels.deinit();
        self.deleted_rels.deinit();
    }
...
pub const LocalStorage = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, txn_id: u64) Self {
        return .{
            .allocator = allocator,
            .txn_id = txn_id,
            .local_node_tables = std.AutoHashMap(TableID, LocalNodeTable).init(allocator),
            .local_rel_tables = std.AutoHashMap(TableID, LocalRelTable).init(allocator),
            .updates = std.ArrayList(LocalUpdate).init(allocator),
            .is_dirty = false,
        };
    }
    ...
    pub fn insertNode(self: *Self, table_id: TableID) !RowOffset {
        ...
        try self.updates.append(LocalUpdate.initInsert(offset));
        self.is_dirty = true;
        return offset;
    }
    ...
    pub fn deleteNode(self: *Self, table_id: TableID, row_offset: RowOffset) !void {
        ...
        try self.updates.append(LocalUpdate.initDelete(row_offset));
        self.is_dirty = true;
    }
