//! storage/table/list_column parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/list_column";
}

test "list_column module path" {
    try std.testing.expectEqualStrings("storage/table/list_column", modulePath());
}

test "list_column resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/list_column"));
}
