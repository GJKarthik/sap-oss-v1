//! storage/table/update_info parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/update_info";
}

test "update_info module path" {
    try std.testing.expectEqualStrings("storage/table/update_info", modulePath());
}

test "update_info resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/update_info"));
}
