//! storage/table/version_info parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/version_info";
}

test "version_info module path" {
    try std.testing.expectEqualStrings("storage/table/version_info", modulePath());
}

test "version_info resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/version_info"));
}
