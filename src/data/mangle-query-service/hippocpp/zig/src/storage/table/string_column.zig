//! storage/table/string_column parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/string_column";
}

test "string_column module path" {
    try std.testing.expectEqualStrings("storage/table/string_column", modulePath());
}

test "string_column resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/string_column"));
}
