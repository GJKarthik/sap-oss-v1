//! storage/table/column_chunk parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/column_chunk";
}

test "column_chunk module path" {
    try std.testing.expectEqualStrings("storage/table/column_chunk", modulePath());
}

test "column_chunk resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/column_chunk"));
}
