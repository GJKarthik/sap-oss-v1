//! storage/table/column_chunk_stats parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/column_chunk_stats";
}

test "column_chunk_stats module path" {
    try std.testing.expectEqualStrings("storage/table/column_chunk_stats", modulePath());
}

test "column_chunk_stats resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/column_chunk_stats"));
}
