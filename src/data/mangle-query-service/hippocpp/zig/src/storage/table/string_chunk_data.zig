//! storage/table/string_chunk_data parity module.
...
pub fn modulePath() []const u8 {
    return "storage/table/string_chunk_data";
}

test "string_chunk_data module path" {
    try std.testing.expectEqualStrings("storage/table/string_chunk_data", modulePath());
}

test "string_chunk_data resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("storage/table/string_chunk_data"));
}
