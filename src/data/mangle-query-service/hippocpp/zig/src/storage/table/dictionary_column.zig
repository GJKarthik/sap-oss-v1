...
pub const StringDictionary = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            .allocator = allocator,
            .string_to_id = std.StringHashMap(u32).init(allocator),
            .id_to_string = std.ArrayList([]const u8).init(allocator),
    ...
    pub fn insert(self: *Self, str: []const u8) !u32 {
        ...
        try self.id_to_string.append(owned);
...
pub const DictionaryChunk = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            ...
            .encoded_values = std.ArrayList(u32).init(allocator),
    ...
    pub fn append(self: *Self, value: ?[]const u8) !void {
        if (value) |v| {
            ...
            try self.encoded_values.append(id);
            try self.null_mask.append(false);
        } else {
            try self.encoded_values.append(0);
            try self.null_mask.append(true);
        }
    }
...
pub const DictionaryColumn = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, column_idx: u32) Self {
        return .{
            ...
            .chunks = std.ArrayList(DictionaryChunk).init(allocator),
    ...
    pub fn addChunk(self: *Self) !*DictionaryChunk {
        try self.chunks.append(DictionaryChunk.init(self.allocator));
...
test "dictionary chunk" {
    ...
    try chunk.append("alice");
    try chunk.append("bob");
    try chunk.append("alice"); // Duplicate
    try chunk.append(null);    // NULL
