...
pub const BoundTable = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, name: []const u8, table_id: u64) BoundTable {
        return .{
            ...
            .table_id = table_id,
            .table_idx = 0,
            .columns = std.ArrayList(BoundColumn).init(allocator),
        };
    }
    ...
    pub fn addColumn(self: *BoundTable, col: BoundColumn) !void {
        ...
        try self.columns.append(new_col);
    }
...
pub const BindingScope = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            .allocator = allocator,
            .tables = std.ArrayList(BoundTable).init(allocator),
            .parent = null,
            .group_by_columns = std.ArrayList(BoundColumn).init(allocator),
        };
    }
    ...
    pub fn addGroupByColumn(self: *Self, col: BoundColumn) !void {
        try self.group_by_columns.append(col);
    }
...
pub const BoundExpression = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, expr_type: BoundExpressionType, data_type: LogicalType) Self {
        return .{
            ...
            .expr_type = expr_type,
            .data_type = data_type,
            .children = std.ArrayList(*BoundExpression).init(allocator),
        };
    }
...
pub const BoundStatement = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, statement_type: ast.StatementType) Self {
        return .{
            ...
            .bound_tables = std.ArrayList(BoundTable).init(allocator),
            .projections = std.ArrayList(*BoundExpression).init(allocator),
            .result_columns = std.ArrayList(BoundColumn).init(allocator),
            .joins = std.ArrayList(BoundJoin).init(allocator),
            .group_by = std.ArrayList(*BoundExpression).init(allocator),
            .order_by = std.ArrayList(OrderByItem).init(allocator),
        };
    }
...
pub const Binder = struct {
    ...
    fn bindQuery(self: *Self, stmt: *const ParsedStatement) BinderError!BoundStatement {
        ...
        if (query.group_by_clause) |group_by| {
            for (group_by.items) |gb_expr| {
                ...
                try result.group_by.append(bound);
        ...
        if (query.select_clause) |select| {
            ...
            for (select.projections.items, 0..) |proj, i| {
                ...
                try result.projections.append(bound_expr);
                ...
                try result.result_columns.append(col);
            }
    ...
    fn bindFromClause(self: *Self, from: *const ast.FromClause, result: *BoundStatement) BinderError!void {
        for (from.tables.items, 0..) |table_ref, i| {
            ...
            try result.bound_tables.append(bound_table);
        }
    ...
    fn bindJoin(self: *Self, join_info: ast.JoinInfo, result: *BoundStatement) BinderError!void {
        ...
        try result.joins.append(bound_join);
    }
    ...
    fn addDefaultColumnsForTable(self: *Self, table: *BoundTable) !void {
