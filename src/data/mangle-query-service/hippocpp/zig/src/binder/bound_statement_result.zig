//! binder/bound_statement_result parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bound_statement_result";
}

test "bound_statement_result binding path" {
    try std.testing.expectEqualStrings("binder/bound_statement_result", bindingPath());
}

test "bound_statement_result effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bound_statement_result"));
}
