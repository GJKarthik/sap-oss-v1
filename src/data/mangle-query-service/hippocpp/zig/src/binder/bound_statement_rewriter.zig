//! binder/bound_statement_rewriter parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bound_statement_rewriter";
}

test "bound_statement_rewriter binding path" {
    try std.testing.expectEqualStrings("binder/bound_statement_rewriter", bindingPath());
}

test "bound_statement_rewriter effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bound_statement_rewriter"));
}
