//! binder/visitor/default_type_solver parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/visitor/default_type_solver";
}

test "default_type_solver binding path" {
    try std.testing.expectEqualStrings("binder/visitor/default_type_solver", bindingPath());
}

test "default_type_solver effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/visitor/default_type_solver"));
}
