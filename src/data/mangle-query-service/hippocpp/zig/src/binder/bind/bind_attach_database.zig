//! binder/bind/bind_attach_database parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_attach_database";
}

test "bind_attach_database binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_attach_database", bindingPath());
}

test "bind_attach_database effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_attach_database"));
}
