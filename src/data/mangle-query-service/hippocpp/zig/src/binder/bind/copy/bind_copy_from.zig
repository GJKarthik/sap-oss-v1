//! binder/bind/copy/bind_copy_from parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/copy/bind_copy_from";
}

test "bind_copy_from binding path" {
    try std.testing.expectEqualStrings("binder/bind/copy/bind_copy_from", bindingPath());
}

test "bind_copy_from effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/copy/bind_copy_from"));
}
