//! binder/bind/bind_graph_pattern parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_graph_pattern";
}

test "bind_graph_pattern binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_graph_pattern", bindingPath());
}

test "bind_graph_pattern effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_graph_pattern"));
}
