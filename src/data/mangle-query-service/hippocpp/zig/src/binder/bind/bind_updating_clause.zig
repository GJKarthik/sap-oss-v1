//! binder/bind/bind_updating_clause parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_updating_clause";
}

test "bind_updating_clause binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_updating_clause", bindingPath());
}

test "bind_updating_clause effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_updating_clause"));
}
