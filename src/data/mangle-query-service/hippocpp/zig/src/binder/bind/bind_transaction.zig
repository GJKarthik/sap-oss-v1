//! binder/bind/bind_transaction parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_transaction";
}

test "bind_transaction binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_transaction", bindingPath());
}

test "bind_transaction effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_transaction"));
}
