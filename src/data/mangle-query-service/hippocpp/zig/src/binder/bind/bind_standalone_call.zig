//! binder/bind/bind_standalone_call parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_standalone_call";
}

test "bind_standalone_call binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_standalone_call", bindingPath());
}

test "bind_standalone_call effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_standalone_call"));
}
