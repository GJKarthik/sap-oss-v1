//! binder/bind/bind_import_database parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_import_database";
}

test "bind_import_database binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_import_database", bindingPath());
}

test "bind_import_database effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_import_database"));
}
