//! binder/bind/bind_projection_clause parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_projection_clause";
}

test "bind_projection_clause binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_projection_clause", bindingPath());
}

test "bind_projection_clause effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_projection_clause"));
}
