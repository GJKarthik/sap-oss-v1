//! binder/bind/read/bind_unwind parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/read/bind_unwind";
}

test "bind_unwind binding path" {
    try std.testing.expectEqualStrings("binder/bind/read/bind_unwind", bindingPath());
}

test "bind_unwind effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/read/bind_unwind"));
}
