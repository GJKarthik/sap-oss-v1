//! binder/bind/bind_table_function parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/bind_table_function";
}

test "bind_table_function binding path" {
    try std.testing.expectEqualStrings("binder/bind/bind_table_function", bindingPath());
}

test "bind_table_function effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/bind_table_function"));
}
