//! binder/bind/copy/bind_copy_to parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/bind/copy/bind_copy_to";
}

test "bind_copy_to binding path" {
    try std.testing.expectEqualStrings("binder/bind/copy/bind_copy_to", bindingPath());
}

test "bind_copy_to effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/bind/copy/bind_copy_to"));
}
