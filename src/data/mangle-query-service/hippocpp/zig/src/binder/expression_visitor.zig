//! binder/expression_visitor parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/expression_visitor";
}

test "expression_visitor binding path" {
    try std.testing.expectEqualStrings("binder/expression_visitor", bindingPath());
}

test "expression_visitor effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/expression_visitor"));
}
