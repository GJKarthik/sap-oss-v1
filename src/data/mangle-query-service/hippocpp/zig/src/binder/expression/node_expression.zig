//! binder/expression/node_expression parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/expression/node_expression";
}

test "node_expression binding path" {
    try std.testing.expectEqualStrings("binder/expression/node_expression", bindingPath());
}

test "node_expression effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/expression/node_expression"));
}
