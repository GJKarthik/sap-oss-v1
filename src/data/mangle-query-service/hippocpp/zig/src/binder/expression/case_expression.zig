//! binder/expression/case_expression parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/expression/case_expression";
}

test "case_expression binding path" {
    try std.testing.expectEqualStrings("binder/expression/case_expression", bindingPath());
}

test "case_expression effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/expression/case_expression"));
}
