//! binder/expression/expression parity module.
...
pub fn bindingPath() []const u8 {
    return "binder/expression/expression";
}

test "expression binding path" {
    try std.testing.expectEqualStrings("binder/expression/expression", bindingPath());
}

test "expression effective phase fallback" {
    var unit = BindingUnit.init("");
    try std.testing.expect(unit.phaseMatches("binder/expression/expression"));
}
