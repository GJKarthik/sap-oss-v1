...
pub const Frontier = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Frontier {
        return .{
            .allocator = allocator,
            .current = std.ArrayList(u64).init(allocator),
            .next = std.ArrayList(u64).init(allocator),
...
pub const GDSState = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) GDSState {
        return .{
            ...
            .parents = std.AutoHashMap(u64, u64).init(allocator),
...
pub const BFSResult = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) BFSResult {
        return .{
            .allocator = allocator,
            .visited = std.ArrayList(u64).init(allocator),
            .distances = std.AutoHashMap(u64, u32).init(allocator),
            .parents = std.AutoHashMap(u64, u64).init(allocator),
        };
    }
...
pub fn executeBFS(
    ...
    try result.visited.append(source);
            ...
            if (adjacency.get(node)) |neighbors| {
                for (neighbors.items) |neighbor| {
                    if (try frontier.addNext(neighbor)) {
                        try result.visited.append(neighbor);
