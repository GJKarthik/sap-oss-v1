...
pub const Path = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Path {
        return .{
            ...
            .edges = std.ArrayList(u64).init(allocator),
        };
    }
    ...
    pub fn clone(self: *const Path, allocator: std.mem.Allocator) !Path {
        ...
        try new_path.edges.appendSlice(self.edges.items);
...
pub const VarPathExecutor = struct {
    ...
    pub fn execute(self: *VarPathExecutor, source: u64, target: ?u64) !VarPathResult {
        ...
        try initial.nodes.append(source);
    ...
    fn dfs(
        ...
        if (self.adjacency.get(current_node)) |edges| {
            for (edges.items) |edge_info| {
                ...
                try new_path.nodes.append(edge_info.target_node);
                try new_path.edges.append(edge_info.edge_id);
...
pub fn allShortestPaths(
    ...
    var current_level = std.ArrayList(Path).init(allocator);
    defer {
        for (current_level.items) |*p| p.deinit();
        current_level.deinit();
    }
    
    var next_level = std.ArrayList(Path).init(allocator);
    defer {
        for (next_level.items) |*p| p.deinit();
        next_level.deinit();
    }
    ...
    try initial.nodes.append(source);
    try current_level.append(initial);
    ...
    while (current_level.items.len > 0 and !found) {
        ...
        for (current_level.items) |*path| {
            ...
            if (end_node == target) {
                ...
                try result.paths.append(complete);
            } else if (!found) {
                // Expand path
                if (adjacency.get(end_node)) |neighbors| {
                    for (neighbors.items) |neighbor| {
                        if (!path.containsNode(neighbor)) {
                            ...
                            errdefer new_path.deinit();
                            try new_path.nodes.append(neighbor);
                            try next_level.append(new_path);
                        }
                    }
                }
            }
...
test "path operations" {
    ...
    try path.nodes.append(1);
    try path.nodes.append(2);
    try path.nodes.append(3);
    try path.edges.append(100);
    try path.edges.append(101);
...
test "path clone" {
    ...
    try original.nodes.append(1);
    try original.nodes.append(2);
    try original.edges.append(100);
...
test "var path executor simple" {
    ...
    // Build graph: 1 -> 2 -> 3 -> 4
    var adj = std.AutoHashMap(u64, std.ArrayList(EdgeInfo)).init(allocator);
    defer {
    ...
    var n1 = std.ArrayList(EdgeInfo).init(allocator);
    try n1.append(.{ .edge_id = 100, .target_node = 2 });
    try adj.put(1, n1);
    
    var n2 = std.ArrayList(EdgeInfo).init(allocator);
    try n2.append(.{ .edge_id = 101, .target_node = 3 });
    try adj.put(2, n2);
    
    var n3 = std.ArrayList(EdgeInfo).init(allocator);
    try n3.append(.{ .edge_id = 102, .target_node = 4 });
    try adj.put(3, n3);
