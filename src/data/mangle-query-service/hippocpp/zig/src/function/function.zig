...
pub const AggregateState = struct {
    data: [64]u8, // Fixed-size state buffer
    is_initialized: bool,
    is_null: bool,
...
pub const FunctionCatalog = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            .allocator = allocator,
            .scalar_functions = std.StringHashMap(std.ArrayList(ScalarFunction)).init(allocator),
            .aggregate_functions = std.StringHashMap(std.ArrayList(AggregateFunction)).init(allocator),
        };
    }
    ...
    pub fn registerScalar(self: *Self, func: ScalarFunction) !void {
        ...
        if (self.scalar_functions.getPtr(name)) |list| {
            try list.append(func);
        } else {
            var list = std.ArrayList(ScalarFunction).init(self.allocator);
            try list.append(func);
    ...
    pub fn registerAggregate(self: *Self, func: AggregateFunction) !void {
        ...
        if (self.aggregate_functions.getPtr(name)) |list| {
            ...
            var list = std.ArrayList(AggregateFunction).init(self.allocator);
            try list.append(func);
