//! function/utility/error parity module.
...
pub fn modulePath() []const u8 {
    return "function/utility/error";
}

test "error module path" {
    try std.testing.expectEqualStrings("function/utility/error", modulePath());
}

test "error canonical name fallback" {
    var module = Module.init("");
    try std.testing.expect(module.matches("function/utility/error"));
}
