...
pub const Parser = struct {
    ...
    fn parseGroupByList(self: *Self, query: *QueryStatement) ParseError!void {
        while (true) {
            const expr = try self.parseOrExpression();
            try query.group_by.append(expr);
            if (!self.match(.COMMA)) break;
        }
    }
    ...
    fn parseOrderByList(self: *Self, query: *QueryStatement) ParseError!void {
        while (true) {
            ...
            try query.order_by.append(item);
    ...
    fn parseComparisonExpression(self: *Self) ParseError!*ParsedExpression {
        var left = try self.parseAddExpression();
        ...
        if (self.match(.BETWEEN)) {
            ...
            expr.left = left;
            expr.args = std.ArrayList(*ParsedExpression).init(self.allocator);
            try expr.args.?.append(low);
            try expr.args.?.append(high);
            return expr;
        }
        ...
        if (self.match(.IN)) {
            ...
            expr.expr_type = .IN_LIST;
            expr.left = left;
            expr.args = std.ArrayList(*ParsedExpression).init(self.allocator);
            
            while (!self.check(.RPAREN) and !self.check(.EOF)) {
                const val = try self.parseOrExpression();
                try expr.args.?.append(val);
                if (!self.match(.COMMA)) break;
            }
    ...
    fn parseFunctionOrIdentifier(self: *Self) ParseError!*ParsedExpression {
        ...
        if (self.match(.LPAREN)) {
            ...
            expr.expr_type = if (is_agg) .AGGREGATE else .FUNCTION_CALL;
    ...
    fn parseCaseExpression(self: *Self) ParseError!*ParsedExpression {
        ...
        while (self.match(.WHEN)) {
            ...
            try expr.args.?.append(condition);
            try expr.args.?.append(result);
        }
    ...
    fn parseInsert(self: *Self) ParseError!ParsedStatement {
        ...
        if (self.match(.LPAREN)) {
            while (!self.check(.RPAREN) and !self.check(.EOF)) {
                ...
                try stmt.insert.?.columns.append(col);
                if (!self.match(.COMMA)) break;
            }
            try self.expect(.RPAREN);
        }
        ...
        while (true) {
            try self.expect(.LPAREN);
            var row = std.ArrayList(*ParsedExpression).init(self.allocator);
            ...
            try self.expect(.RPAREN);
            try stmt.insert.?.values.append(row);
    ...
    fn parseUpdate(self: *Self) ParseError!ParsedStatement {
        ...
        while (true) {
            ...
            const val = try self.parseOrExpression();
            try stmt.update.?.assignments.append(.{ .column = col, .value = val });
            if (!self.match(.COMMA)) break;
        }
        ...
        while (!self.check(.RPAREN) and !self.check(.EOF)) {
            ...
            while (true) {
                if (self.match(.PRIMARY)) {
                    ...
                    col_def.default_value = try self.parseOrExpression();
                } else break;
            }
    ...
    fn parseCreateIndex(self: *Self) ParseError!ParsedStatement {
        ...
        var stmt = ParsedStatement.createIndex(self.allocator, index_name, table_name);
        
        try self.expect(.LPAREN);
        while (!self.check(.RPAREN) and !self.check(.EOF)) {
            const col = try self.expectIdentifier();
            try stmt.create_index.?.columns.append(col);
            if (!self.match(.COMMA)) break;
        }
        try self.expect(.RPAREN);
    ...
    fn parseDrop(self: *Self) ParseError!ParsedStatement {
        ...
        if (self.match(.TABLE)) {
            ...
            _ = self.match(.EXISTS);
            const index_name = try self.expectIdentifier();
            return ParsedStatement.createDropIndex(self.allocator, index_name);
        }
    ...
    fn parseDataType(self: *Self) common.LogicalType {
        const dt: common.LogicalType = switch (self.current.token_type) {
            .INT, .INT32 => .INT32,
            .INT64 => .INT64,
            .FLOAT_TYPE, .REAL => .FLOAT,
            .DOUBLE => .DOUBLE,
            .VARCHAR, .TEXT, .STRING_TYPE => .STRING,
            .BOOLEAN, .BOOL => .BOOL,
            .DATE => .DATE,
            .TIMESTAMP => .TIMESTAMP,
            else => .ANY,
    ...
    fn parsePattern(self: *Self, match_stmt: *ast.MatchStatement) ParseError!void {
        while (true) {
            ...
            if (self.match(.LPAREN)) {
                ...
                try self.expect(.RPAREN);
                try match_stmt.nodes.append(node);
            }
            ...
            if (self.check(.MINUS) or self.check(.LESS_THAN)) {
                ...
                try match_stmt.edges.append(edge);
                continue;
            }
