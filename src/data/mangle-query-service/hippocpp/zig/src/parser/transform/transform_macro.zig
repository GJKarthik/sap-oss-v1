//! parser/transform/transform_macro parity module.
...
pub fn modulePath() []const u8 {
    return "parser/transform/transform_macro";
}

test "transform_macro module path" {
    try std.testing.expectEqualStrings("parser/transform/transform_macro", modulePath());
}

test "transform_macro resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("parser/transform/transform_macro"));
}
