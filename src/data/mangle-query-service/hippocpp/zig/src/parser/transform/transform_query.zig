//! parser/transform/transform_query parity module.
...
pub fn modulePath() []const u8 {
    return "parser/transform/transform_query";
}

test "transform_query module path" {
    try std.testing.expectEqualStrings("parser/transform/transform_query", modulePath());
}

test "transform_query resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("parser/transform/transform_query"));
}
