//! parser/transform/transform_ddl parity module.
...
pub fn modulePath() []const u8 {
    return "parser/transform/transform_ddl";
}

test "transform_ddl module path" {
    try std.testing.expectEqualStrings("parser/transform/transform_ddl", modulePath());
}

test "transform_ddl resolved tag fallback" {
    var node = ParityNode.init("");
    try std.testing.expect(node.tagMatches("parser/transform/transform_ddl"));
}
