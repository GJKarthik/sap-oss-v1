...
def _create_connection(kuzu: Any, database: Any, num_threads: int) -> Any:
    if num_threads > 0:
        try:
            return kuzu.Connection(database, num_threads=num_threads)
        except TypeError:
            pass
        try:
            return kuzu.Connection(database, num_threads)
        except TypeError:
            pass
    return kuzu.Connection(database)


def _run_case(kuzu: Any, case: dict[str, Any], db_path: Path, query_threads: int) -> dict[str, Any]:
    database = kuzu.Database(str(db_path))
    connection = _create_connection(kuzu, database, query_threads)
    ...
    try:
        if query_threads > 0 and hasattr(connection, "set_max_threads_for_exec"):
            try:
                connection.set_max_threads_for_exec(query_threads)
            except Exception:
                pass
...
def main() -> int:
    ...
    query_threads = int(os.environ.get("HIPPOCPP_QUERY_THREADS", "1"))
    if query_threads < 0:
        query_threads = 1
    ...
    for idx, case in enumerate(cases):
        ...
        if db_path.exists():
            if db_path.is_dir():
                ...
                db_path.unlink()
        run_results.append(_run_case(kuzu, case, db_path, query_threads))
