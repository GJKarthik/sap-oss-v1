...
def _run_backend(name: str, cmd_template: str, corpus_path: Path, timeout_seconds: float | None) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "runner_status": "error",
            "runner_error": f"Command timed out after {timeout_seconds} seconds",
            "command": command,
...
def main() -> int:
    ...
    parser.add_argument(
        "--timeout-seconds",
        type=float,
        default=float(os.environ.get("HIPPOCPP_DIFF_TIMEOUT_SECONDS", "120")),
        help="Per-backend command timeout in seconds. Use <=0 to disable timeout.",
    )
    args = parser.parse_args()
    timeout_seconds: float | None = args.timeout_seconds if args.timeout_seconds > 0 else None
    ...
    left = _run_backend(args.left_name, args.left_cmd, corpus_path, timeout_seconds)
    right = _run_backend(args.right_name, args.right_cmd, corpus_path, timeout_seconds)
