...
async function streamAICoreRequest(config, path, body, res) {
    const token = await getAccessToken(config);
    const baseUrl = new URL(config.baseUrl);
    return new Promise((resolve, reject) => {
        const req = https.request({
...
function findDeployment(deployments, modelId) {
    ...
    if (deployment)
        return deployment;
    // Try partial match
    deployment = deployments.find(d => modelId.toLowerCase().includes(d.id.substring(0, 8)) ||
...
function createServer(options = {}) {
    ...
    app.post('/v1/chat/completions', authMiddleware, async (req, res) => {
        try {
            ...
            const completionId = `chatcmpl-${(0, uuid_1.v4)()}`;
            ...
            if (deployment.isAnthropic) {
                ...
                if (body.stream) {
                    ...
                    res.setHeader('Cache-Control', 'no-cache');
                    res.setHeader('Connection', 'keep-alive');
                    await streamAICoreRequest(config, `/v2/inference/deployments/${deployment.id}/chat/completions`, { ...body, stream: true }, res);
                }
                else {
                    const result = await aiCoreRequest(config, 'POST', `/v2/inference/deployments/${deployment.id}/chat/completions`, body);
                    res.json({
                        id: completionId,
                        ...result,
                        model: deployment.id,
    ...
    app.post('/v1/embeddings', authMiddleware, async (req, res) => {
        try {
            ...
            if (!deployment) {
                ...
                deployment = deployments.find(d => d.model.includes('embed') || d.model.includes('ada'));
            }
            if (!deployment) {
                return res.status(400).json({
        ...
        try {
            ...
            try {
                ...
                if (embeddingDeployment) {
                    const result = await aiCoreRequest(config, 'POST', `/v2/inference/deployments/${embeddingDeployment.id}/embeddings`, {
                        input: [file],
                    });
