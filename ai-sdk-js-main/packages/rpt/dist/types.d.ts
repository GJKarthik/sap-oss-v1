 ...
 */
type ColNames<T extends DataSchema> = T extends readonly any[] ? T[number]['name'] : string;
/**
 ...
 * @template T - Type of the data schema.
 */
type TsType<T extends ColumnType> = T extends 'numeric' ? number : T extends 'date' ? DateString : string;
/**
...
type RowType<T extends DataSchema> = T extends readonly any[] ? {
    [N in T[number]['name']]: TsType<Extract<T[number], {
        name: N;
    }>['dtype']>;
