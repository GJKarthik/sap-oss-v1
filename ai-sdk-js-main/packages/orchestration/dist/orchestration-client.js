...
import { constructCompletionPostRequest, constructCompletionPostRequestFromJsonModuleConfig, constructCompletionPostRequestFromConfigReference } from './util/index.js';
import { isConfigReference, isOrchestrationModuleConfigList, assertIsOrchestrationModuleConfigList } from './orchestration-types.js';
...
export class OrchestrationClient {
    ...
    async chatCompletion(request, requestConfig) {
        ...
        if (isConfigReference(this.config) && request?.messages?.length) {
            logger.warn('The messages field in request is not supported when using an orchestration config reference. Messages should be part of the referenced configuration or provided via messagesHistory. The messages field will be ignored.');
        }
    ...
    async stream(request, signal, options, requestConfig) {
        if (isOrchestrationModuleConfigList(this.config)) {
            throw new Error('Streaming is not supported when using multiple orchestration module configurations for fallback. Please use a single configuration.');
        }
        ...
        try {
            ...
            if (isConfigReference(this.config)) {
                if (options) {
                    logger.warn('Stream options are not supported when using an orchestration config reference. Streaming is only supported if the referenced config has streaming configured.');
                }
    ...
    async executeRequest(options) {
        ...
        const body = typeof this.config === 'string'
            ...
            : isConfigReference(this.config)
                ? constructCompletionPostRequestFromConfigReference(this.config, request)
        ...
        if (!stream && response.data?.intermediate_failures) {
            ...
            logger.info(`Orchestration used ${failureCount} fallback(s) before success${successModel ? `. Succeeded with model: ${successModel}` : ''}.`);
        }
        return response;
    }
    async createStreamResponse(options, controller) {
        ...
        const response = new OrchestrationStreamResponse(streamResponse);
    ...
    parseModuleConfigList(config) {
        ...
        const parsedConfigs = config.map(c => this.parseTemplatingModule(c));
        ...
        if (uniqueModels.size < models.length) {
            logger.warn(`Fallback configurations contain duplicate models: [${models.join(', ')}]. ` +
