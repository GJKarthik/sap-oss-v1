...
function getMockObject({ item, mockOptions, operationId, tags, combine, context, imports, existingReferencedProperties, splitMockImplementations, allowOverride = false }) {
	...
	if (item.properties) {
		...
		const propertyScalars = entries.map(([key, prop$1]) => {
			...
			const hasNullable = "nullable" in prop$1 && prop$1.nullable === true;
			if ("$ref" in prop$1 && existingReferencedProperties.includes(pascal(prop$1.$ref.split("/").pop() ?? ""))) {
			...
			const hasDefault = "default" in prop$1 && prop$1.default !== void 0;
			...
			if (Array.isArray(prop$1.type) && prop$1.type.includes("null") && !resolvedValue.overrided) return `${keyDefinition}: faker.helpers.arrayElement([${resolvedValue.value}, null])`;
