import { camel, compareVersions, escape, generateMutator, getFormDataFieldFileType, getNumberWord, getPropertySafe, getRefInfo, isBoolean, isNumber, isObject, isString, jsStringEscape, pascal, resolveRef, stringify } from "@orval/core";
...
const generateZodValidationSchemaDefinition = (schema, context, name, strict, isZodV4, rules) => {
	...
	if (schema.allOf || schema.oneOf || schema.anyOf) {
		...
		const baseSchemas = (schema.allOf ?? schema.oneOf ?? schema.anyOf).map((schema$1, index) => generateZodValidationSchemaDefinition(schema$1, context, `${camel(name)}${pascal(getNumberWord(index + 1))}`, strict, isZodV4, { required: true }));
...
const parseZodValidationSchemaDefinition = (input, context, coerceTypes = false, strict, isZodV4, preprocess) => {
	...
	const formatFunctionArgs = (value$1) => {
		if (value$1 === void 0) return "";
		if (value$1 === null) return "null";
		if (isString(value$1)) return value$1;
		if (Array.isArray(value$1)) return value$1.map((item) => formatFunctionArgs(item)).join(", ");
		if (isObject(value$1)) return stringify(value$1) ?? "";
		if (isNumber(value$1) || isBoolean(value$1)) return `${value$1}`;
		return "";
	};
...
${Object.entries(mergedProperties).map(([key, schema$1]) => {
					const value$1 = schema$1.functions.map((prop) => parseProperty(prop)).join("");
					appendConstsChunk(schema$1.consts.join("\n"));
			...
			let acc = "";
			for (const partSchema of allOfArgs) {
				const value$1 = partSchema.functions.map((prop) => parseProperty(prop)).join("");
		...
		if (fn === "oneOf" || fn === "anyOf") {
			...
			return `.union([${unionArgs.map(({ functions, consts: argConsts }) => {
				const value$1 = functions.map((prop) => parseProperty(prop)).join("");
				const valueWithZod = `${value$1.startsWith(".") ? "zod" : ""}${value$1}`;
		...
		if (fn === "additionalProperties") {
			...
			const value$1 = additionalPropertiesArgs.functions.map((prop) => parseProperty(prop)).join("");
			const valueWithZod = `${value$1.startsWith(".") ? "zod" : ""}${value$1}`;
...
${Object.entries(objectArgs).map(([key, schema$1]) => {
				const value$1 = schema$1.functions.map((prop) => parseProperty(prop)).join("");
				appendConstsChunk(schema$1.consts.join("\n"));
				return `  "${key}": ${value$1.startsWith(".") ? "zod" : ""}${value$1}`;
			}).join(",\n")}
		...
		if (fn === "array") {
			...
			const value$1 = arrayArgs.functions.map((prop) => parseProperty(prop)).join("");
		...
		if (fn === "tuple") return `zod.tuple([${args.map((x) => {
			const value$1 = x.functions.map((prop) => parseProperty(prop)).join("");
...
const dereference = (schema, context) => {
	...
	const resolvedSchema = "$ref" in schema ? (() => {
		const referencedSchema = context.spec.components?.schemas?.[getRefInfo(schema.$ref, context).name];
...
//#endregion
export { builder, src_default as default, dereference, generateZod, generateZodValidationSchemaDefinition, getZodDependencies, isZodVersionV4, parseParameters, parseZodValidationSchemaDefinition };
