...
pub const HanaConfig = struct {
    ...
    pub fn fromEnv(allocator: std.mem.Allocator, destination_name: []const u8) !HanaConfig {
        ...
        return HanaConfig{
            .host = std.posix.getenv("HANA_HOST") orelse try allocator.dupe(u8, "localhost"),
            .port = if (std.posix.getenv("HANA_PORT")) |p|
                std.fmt.parseInt(u16, p, 10) catch 443
            else
                443,
            .schema = std.posix.getenv("HANA_SCHEMA") orelse try allocator.dupe(u8, "SYSTEM"),
            .user = std.posix.getenv("HANA_USER") orelse try allocator.dupe(u8, "SYSTEM"),
            .password = std.posix.getenv("HANA_PASSWORD") orelse try allocator.dupe(u8, ""),
...
pub const HanaConnector = struct {
    ...
    pub fn resolveGraph(self: *HanaConnector, ptr: *const toon_pointer.ToonPointer) !toon_pointer.PointerResolution {
        ...
        var hint = std.ArrayList(u8).init(self.allocator);
